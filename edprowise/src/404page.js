...
const Styled404 = styled.div`
  .error-container {
    background: ${props => props.darkMode ? '#1a1a1a' : 'linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%)'};
    min-height: 100vh;
    ...
    <Styled404 darkMode={darkMode}>
      <div className="error-container d-flex flex-column justify-content-center align-items-center p-3">
        ...
        >
          ...
          <h1 className="display-1 fw-bold mb-3" style={{ color: darkMode ? '#fff' : '#2c3e50' }}>
          ...
          <div className="d-flex gap-3 justify-content-center">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className={`dynamic-button btn btn-lg ${darkMode ? 'btn-light' : 'btn-primary'}`}
              onClick={() => navigate(-1)}
            >
              ← Go Back
            </motion.button>
            ...
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className={`dynamic-button btn btn-lg ${darkMode ? 'btn-primary' : 'btn-dark'}`}
              onClick={() => navigate('/')}
            >
              🏠 Home
            </motion.button>
          ...
          <div className="mt-4">
            <p className={`small ${darkMode ? 'text-light' : 'text-muted'}`}>
              Need help? <Link to="/contact" className={darkMode ? 'text-light' : 'text-dark'}>Contact support</Link>
            </p>
          </div>
        </motion.div>
