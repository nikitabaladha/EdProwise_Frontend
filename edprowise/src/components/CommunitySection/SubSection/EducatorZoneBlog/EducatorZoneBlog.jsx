...
const EducatorZoneBlog = () => {
  ...
  const handleCategoryClick = async (categoryId) => {
    try {
      const res = await getAPI(
        `/get-blogs-by-category/${categoryId}?blogFor=Educator Zone`,
        {},
        true
      );
      if (res.success || res.data?.success) {
        ...
        navigate(
          "/community-connect/educator-zone/category-tag-related-blogs",
          {
            state: {
              relatedBlogs,
              categoryId,
  ...
  const handleTagsClick = async (tagId) => {
    try {
      const res = await getAPI(
        `/get-blogs-by-tag/${tagId}?blogFor=Educator Zone`,
        {},
        true
      );
      if (res.success || res.data?.success) {
