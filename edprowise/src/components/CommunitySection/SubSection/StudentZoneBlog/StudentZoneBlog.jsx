...
const StudentZoneBlog = () => {
    ...
    const handleCategoryClick = async (categoryId) => {
        try {
            const res = await getAPI(`/get-blogs-by-category/${categoryId}?blogFor=Student Zone`, {}, true);
            if (res.success || res.data?.success) {
                 const relatedBlogs = res.data?.data || [];
                 navigate("/community-connect/student-zone/category-tag-related-blogs", {
                state: {
                    relatedBlogs,
                    categoryId
                }
     ...
     const handleTagsClick = async (tagId) => {
        try {
            const res = await getAPI(`/get-blogs-by-tag/${tagId}?blogFor=Student Zone`, {}, true);
            if (res.success || res.data?.success) {
                 const relatedBlogs = res.data?.data || [];
                 navigate("/community-connect/student-zone/category-tag-related-blogs", {
                state: {
...
export default StudentZoneBlog;
