...
const AddNewSubscription = ({ addSubscription, schools }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.subscriptionNoOfMonth <= 0 || formData.monthlyRate <= 0) {
      ...
      toast.error("Please enter valid values for number of months and monthly rate.");
    ...
    try {
      ...
      const response = await postAPI("/subscription", {
      ...
      if (!response.hasError) { 
        ...
        // Assuming `addnewSubscription` will be used to update the parent state
