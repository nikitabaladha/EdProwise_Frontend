...
const ViewBlog = () => {
    ...
    const handleCategoryClick = async (categoryId) => {
        try {
            const res = await getAPI(`/get-blogs-by-category/${categoryId}`, {}, true);
            if (res.success || res.data?.success) {
                setRelatedCategoryBlogs(res.data?.data || []);
                console.log("Related blogs fetched:", res.data?.data);
            }
        } catch (err) {
                    ...
                    <div className="row">
                        <div className="col-lg-10 offset-lg-1">
                            <div className="wpo-terms-wrap">
                                        ...
                                        {blog.tags.map((tagObj) => {
                                            ...
                                            return (
                                                <span
                                                    key={tagObj._id}
                                                    className="badge bg-primary text-white me-2"
                                                    onClick={() => handleCategoryClick(tagObj._id)}
                                                    style={{ cursor: 'pointer' }}
                                                >
                                ...
                                {/* Related Blogs (optional display) */}
                                {relatedCategoryBlogs.length > 0 && (
                                    <div className="mt-4">
                                        <h4>Related Blogs:</h4>
                                        <ul>
                                            {relatedCategoryBlogs.map((item) => (
                                                <li key={item._id}>
                                                    <span className="text-primary fw-bold">{item.blogTitle}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
