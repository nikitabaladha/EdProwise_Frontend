...
const AdminRegistrationEmailTamplate = () => {
  const [formData, setFormData] = useState({
    mailFrom: "",
    subject: "Admin Registration Sucessful",
    content: "",
  });
  ...
  useEffect(() => {
    ...
    const fetchEmailTemplate = async () => {
      try {
        const response = await getAPI("/get-admin-registration-templates", {}, true);
        if (!response.hasError) {
          setFormData((prevData) => ({
            ...prevData,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await postAPI("/post-admin-registration-templates", formData, true);
      if (!response.hasError) {
                    ...
                    <div className="col-md-12">
                      <div className="mb-3">
                        ...
                        <input
                          ...
                          defaultValue="Admin Registration Successful"
                          required
                        />
                      </div>
                    </div>
...
export default AdminRegistrationEmailTamplate;
