...
const MarketingEmail = () => {
    ...
    const fetchInitialData = async () => {
      try {
        ...
        if (!emailResponse.hasError) {
          setEmailOptions(emailResponse.data.data.map(email => ({
            label: email,
            value: email,
          })));
        }
    ...
    reader.onload = (e) => {
      try {
        ...
        setEmailOptions(prev => [
          ...prev,
          ...validEmails.map(email => ({ label: email, value: email }))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.attachments.forEach(att => formDataToSend.append("attachments", att.file));
                    ...
                    <div className="mb-3">
                      ...
                      <div className="d-flex flex-wrap gap-2">
                        {formData.attachments.map((file, index) => (
                          <div key={index} className="bg-light p-2 rounded d-flex align-items-center">
