...
const SMTPHostSettings = () => {
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            try {
                const response = await getAPI("/get-smtp-email-settings", {},
                    true);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await postAPI("/post-smtp-email-settings", formData, true);
    ...
    const handleTestMail = async (e) => {
        ...
        try {
            const response = await postAPI("/test-smtp-email-settings", { email: testEmail }, true);
            ...
            {showModal && (
                <div className="modal-overlay">
                    <div className="modal-content">
                        ...
                        <input type="email" className="form-control" placeholder="Enter Email" value={testEmail} onChange={(e) => setTestEmail(e.target.value)} required />
                        <div className="d-flex justify-content-end mt-3">
                            {/* {sending && <label><i className="fas fa-clock"></i> Sending...</label>} */}
                            <button className="btn btn-secondary mr-2" onClick={() => setShowModal(false)}>Cancel</button>
                            {/* <button className="btn btn-primary" style={{ marginLeft: "1rem" }} onClick={handleTestMail}>Send</button> */}
                            <button className="btn btn-primary" style={{ marginLeft: "1rem" }} onClick={handleTestMail} disabled={sending}>
