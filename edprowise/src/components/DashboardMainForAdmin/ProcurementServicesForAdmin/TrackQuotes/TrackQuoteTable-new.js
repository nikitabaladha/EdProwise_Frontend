...
const TrackQuoteTable = () => {
  ...
  useEffect(() => {
    if (location.state?.searchEnquiryNumber) {
      setSearchFilter(location.state.searchEnquiryNumber);
    } else {
      setSearchFilter("");
    }
  }, [location.state]);
  ...
  const [schoolsPerPage] = useState(3);
  ...
  const totalPages = Math.ceil(filteredQuotes.length / schoolsPerPage);
  ...
  useEffect(() => {
    if (searchFilter) {
      const foundIndex = quotes.findIndex((quote) =>
        quote.enquiryNumber.includes(searchFilter)
      );

      if (foundIndex >= 0) {
        const newPage = Math.ceil((foundIndex + 1) / schoolsPerPage);
        setCurrentPage(newPage);
      }
    }
  }, [searchFilter, quotes, schoolsPerPage]);
