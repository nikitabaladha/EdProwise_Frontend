...
const EmployeeLTADetails = () => {
  ...
  const openDeleteDialog = (lta, employeeId) => {
    if (!lta._id) {
      
      toast.error('Cannot delete: Invalid record ID');
      return;
    }
    console.log("openDeleteDialog empid",employeeId);
  ...
  const handleDeleteConfirmed = (detailId) => {
    console.log("detailId",detailId);
    console.log( setEmployeeLtaDetails((prev) => prev.filter((lta) => lta._id !== detailId)));
  };
