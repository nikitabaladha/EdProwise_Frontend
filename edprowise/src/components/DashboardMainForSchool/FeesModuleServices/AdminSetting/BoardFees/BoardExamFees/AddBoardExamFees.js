...
const AddBoardExamFees = () => {
  ...
  const handleSubmitAll = async (e) => {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      try {
        const response = await postAPI("/create-board-exam-fees", payload, {}, true);
        if (response.hasError) {
          allSuccess = false;
          toast.error(`Entry ${i + 1}: ${response.message || "Failed to create board exam fees."}`);
        } else {
          toast.success(`Entry ${i + 1}: ${response.message || "Board exam fees created successfully."}`);
        }
      } catch (err) {
        allSuccess = false;
