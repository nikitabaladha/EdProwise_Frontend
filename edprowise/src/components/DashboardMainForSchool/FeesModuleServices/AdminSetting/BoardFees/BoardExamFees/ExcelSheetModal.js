...
const ExcelSheetModal = ({ show, onClose, schoolId, academicYear, onImportSuccess }) => {
        ...
        reader.onload = (e) => {
          ...
          jsonData.forEach((row, index) => {
            ...
            if (!groupedData[key]) {
              ...
              toast.error(`Row ${index + 1}: Duplicate class "${className}" and sections "${sections.join(', ')}".`);
              hasError = true;
            }
          });
    ...
    for (const [index, payload] of validatedData.entries()) {
      try {
        ...
        if (!response.hasError) {
          toast.success(`Board exam fees for class imported successfully (Row ${index + 1}).`);
        } else {
          toast.error(
            response.message || `Failed to import board exam fees for row ${index + 1}.`
          );
        }
      } catch (err) {
        const errorMessage =
          err?.response?.data?.message || `Error importing board exam fees for row ${index + 1}.`;
  ...
  const handleDownloadDemo = () => {
    ...
    XLSX.writeFile(wb, 'demo_board_exam_fees.xlsx');
  };
