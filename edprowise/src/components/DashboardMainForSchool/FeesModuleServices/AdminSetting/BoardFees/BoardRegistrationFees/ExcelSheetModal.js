...
const ExcelSheetModal = ({ show, onClose, schoolId, academicYear, onImportSuccess }) => {
    ...
    for (const [index, payload] of validatedData.entries()) {
      try {
        ...
        if (!response.hasError) {
          ...
          toast.error(
            response.message || `Failed to import board registration fees for row ${index + 1}.`
          );
        }
      } catch (err) {
        const errorMessage =
          err?.response?.data?.message || `Error importing board registration fees for row ${index + 1}.`;
  ...
  const handleDownloadDemo = () => {
    ...
    XLSX.writeFile(wb, 'demo_board_registration_fees.xlsx');
  };
