...
const UpdateBoardRegistrationFees = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await putAPI(`/update-board-registration-fees/${existingFees._id}`, payload, {}, true);
      if (response.hasError) {
        toast.error(response.message || "Failed to update board registration fees.");
      } else {
        toast.success(response.message || "Board registration fees updated successfully.");
        navigate(-1);
      }
    } catch (err) {
        ...
        <div className="col-xl-12">
          <div className="card m-2">
            <div className="card-body">
              <h4 className="card-title text-center">Update Board Registration Fees</h4>
