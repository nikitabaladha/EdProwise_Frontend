      ...
      try {
        ...
        reader.onload = (e) => {
          ...
          jsonData.forEach((row, index) => {
            ...
            const groupOfFees = row.GroupOfFees ? String(row.GroupOfFees).trim() : '';

            if (!feesTypeName || !groupOfFees) {
              toast.error(`Row ${index + 1}: Fees Type Name or Group of Fees is missing.`);
              hasError = true;
              return;
            }
            ...
            if (!validGroups.includes(groupOfFees)) {
              toast.error(
                `Row ${index + 1}: Invalid Group of Fees "${groupOfFees}". Must be one of: ${validGroups.join(', ')}.`
              );
              hasError = true;
              return;
            }
  ...
  const handleConfirmImport = async () => {
    ...
    for (const [index, payload] of validatedData.entries()) {
      try {
        ...
        const errorMessage =
          err?.response?.data?.message || `Error importing fees type for row ${index + 1}.`;
