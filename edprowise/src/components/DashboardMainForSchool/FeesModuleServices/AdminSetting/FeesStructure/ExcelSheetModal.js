...
const ExcelSheetModal = ({ show, onClose, schoolId, academicYear, onImportSuccess }) => {
  ...
  const handleImportClick = () => {
    ...
    if (feesTypes.length === 0) {
      toast.error('No school fees types available. Please configure fees types.');
      return;
    }
        ...
        reader.onload = (e) => {
          ...
          jsonData.forEach((row, index) => {
            ...
            if (!installment) {
              ...
              groupedData[key].installments.push(installment);
            }

            installment.fees.push({
    ...
    for (const [index, payload] of validatedData.entries()) {
      try {
        ...
        if (!response.hasError) {
          toast.success(`Fee structure for class imported successfully (Row ${index + 1}).`);
        } else {
          toast.error(
            response.message || `Failed to import fee structure for row ${index + 1}.`
          );
        }
      } catch (err) {
        const errorMessage =
          err?.response?.data?.message || `Error importing fee structure for row ${index + 1}.`;
  ...
  const handleDownloadDemo = () => {
    ...
    const wsData = [
      ...
      ['Grade 1', 'A,B', 'Installment 2', '2025-07-01', 'Tuition Fee', 10000],
