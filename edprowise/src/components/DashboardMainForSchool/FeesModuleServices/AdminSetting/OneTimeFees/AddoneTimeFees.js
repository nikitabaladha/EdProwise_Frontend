    ...
    for (let i = 0; i < forms.length; i++) {
      ...
      try {
        ...
        if (response.hasError) {
          allSuccess = false;
          toast.error(`Form ${i + 1}: ${response.message || "Failed to create fees."}`);
        } else {
          toast.success(`Form ${i + 1}: ${response.message || "One-time fees added successfully."}`);
        }
      } catch (err) {
        allSuccess = false;
                              ...
                              {!form.showFeeDetails && (
                                <div className="col-md-3 d-flex align-items-end">
                                  <button
                                    ...
                                    onClick={() => updateForm(index, { showFeeDetails: true })}
                                    disabled={!form.selectedClassId || form.selectedSections.length === 0}
                                  >
                                    Proceed
                                  </button>
                                </div>
                            ...
                            <div className="row mb-2" key={feeIndex}>
                              <div className="col-md-5">
                                ...
                                <select
                                  ...
                                  value={fee.feesType}
                                  onChange={(e) =>
                                    handleFeeChange(index, feeIndex, "feesType", e.target.value)
                                  }
                                >
                                  <option value="">Select Fee Type</option>
                                  {feesTypesList
                                    .filter((type) => {
                                      const selectedFeeTypes = form.feesDetails.map((f) => f.feesType);
                                      return (
                              ...
                              <div className="col-md-5">
                                ...
                                <input
                                  ...
                                  value={fee.amount}
                                  onChange={(e) =>
                                    handleFeeChange(index, feeIndex, "amount", e.target.value)
                                  }
                                />
                              </div>
