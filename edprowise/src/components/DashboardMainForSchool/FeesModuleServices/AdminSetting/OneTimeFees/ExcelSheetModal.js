...
const ExcelSheetModal = ({ show, onClose, schoolId, academicYear, onImportSuccess }) => {
  ...
  const handleImportClick = () => {
    ...
    if (feesTypes.length === 0) {
      toast.error('No one-time fees types available. Please configure fees types.');
      return;
    }
          ...
          jsonData.forEach((row, index) => {
            ...
            if (validSections.length !== sections.length) {
              const invalid = sections.filter(
                (s) => !classObj.sections.some((sec) => sec.name.toLowerCase() === s.toLowerCase())
              );
              toast.error(`Row ${index + 1}: Invalid section(s) "${invalid.join(', ')}" for class "${className}".`);
              hasError = true;
              return;
            }

            const sectionIds = validSections.map((s) =>
            ...
            if (!feesType) {
              toast.error(`Row ${index + 1}: Invalid fees type "${feesTypeName}".`);
              hasError = true;
              return;
            }
            ...
            groupedData[key].oneTimeFees.push({
              feesTypeId: feesType._id,
              amount,
            });
          });
  ...
  const handleConfirmImport = async () => {
    ...
    for (const [index, payload] of validatedData.entries()) {
      try {
        const response = await postAPI('/create-one-time-fees', payload, true);
        if (!response.hasError) {
          toast.success(`One-time fees for class imported successfully (Row ${index + 1}).`);
        } else {
          toast.error(
            response.message || `Failed to import one-time fees for row ${index + 1}.`
          );
        }
      } catch (err) {
        const errorMessage =
          err?.response?.data?.message || `Error importing one-time fees for row ${index + 1}.`;
