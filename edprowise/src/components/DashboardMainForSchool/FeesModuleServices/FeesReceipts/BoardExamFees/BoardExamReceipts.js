...
//     pdf.save(`fees_receipt_${singleStudent.receiptNumberBef || "unknown"}.pdf`);
...
// export default BoardExamFeesReceipt;
...
const BoardExamFeesReceipt = () => {
  const location = useLocation();
  const { receiptNumberBef, schoolId, className, sectionName, classId, sectionId } = location.state || {};
  const academicYear = localStorage.getItem('selectedAcademicYear');
  ...
  const receiptRef = useRef(null);
  const feeTypeName = "Board Exam Fee";

  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      setError(null);
      console.log("Starting fetchData with:", { schoolId, receiptNumberBef });
      ...
      try {
        const url = receiptNumberBef 
          ? `/get-board-exam-payment-data/${schoolId}/${encodeURIComponent(receiptNumberBef)}`
          : `/get-board-exam-payment-data/${schoolId}`;
        ...
        setError("Failed to fetch board exam payment data: " + error.message);
        toast.error("Failed to fetch board exam payment data: " + error.message);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const downloadReceiptAsPDF = async (singleStudent = students[currentStudentIndex]) => {
    ...
    pdf.addImage(canvas.toDataURL("image/jpeg", 0.98), "JPEG", 0, 0, imgWidth, canvasHeight);
    pdf.save(`board_exam_fees_receipt_${singleStudent.receiptNumberBef || "unknown"}.pdf`);
  ...
  return (
    <div className="container my-4" style={{ maxWidth: "800px" }}>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="text-primary">
          <strong>Board Exam Fees Receipt</strong>
        </h4>
        <div>
          <div className="dropdown me-2 d-inline-block">
            <button
