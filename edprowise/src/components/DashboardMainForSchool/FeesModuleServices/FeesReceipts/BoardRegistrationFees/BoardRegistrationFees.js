...
const BoardFeeRegistration = () => {
    ...
    const fetchStudents = async () => {
        ...
        try {
            ...
            setShowTable(true);
            setSelectedStudentIds([]); // Reset selected students when fetching new students
        } catch (error) {
    ...
    const handlePaidClick = async (event, student) => {
        ...
        try {
            ...
            const receiptData = {
                ...student,
                receiptNumber: response?.data?.data?.[0]?.receiptNumber || `REC-${student._id}-${Date.now()}`,
        ...
        try {
            ...
            const receiptStudents = selectedStudents
                ...
                .map((student, index) => ({
                    ...student,
                    receiptNumber:
                        response?.data?.data?.[index]?.receiptNumber || `REC-${student._id}-${Date.now()}`,
                    admissionFees: student.feesAmt,
                    concessionAmount: 0,
                    finalAmount: student.feesAmt,
                    applicationDate: new Date().toISOString(),
                                ...
                                <table className="table table-hover mb-1 text-nowrap">
                                    <thead className="table-light">
                                        <tr>
                                            ...
                                            <th scope="col" className="text-uppercase small fw-semibold m-2">Adm. No.</th>
                                            ...
                                            {hasChequePayment && (
                                                <>
                                                    <th scope="col" className="text-uppercase small fw-semibold m-2">Cheque No.</th>
                                                    <th scope="col" className="text-uppercase small fw-semibold m-2">Bank Name</th>
                                                </>
                                            )}
                                            <th scope="col" className="text-uppercase small fw-semibold m-2">Status</th>
                                            <th scope="col" className="text-uppercase small fw-semibold m-2">Pay</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {students.map((student) => (
                                            <tr key={student._id}>
                                                            ...
                                                            {student.paymentMode === 'Cheque' ? (
                                                                student.paymentStatus === 'Paid' ? (
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        onChange={(e) => handleChequeFieldChange(student._id, 'chequeNumber', e.target.value)}
                                                                        required
                                                                    />
                                                                )
                                                            ) : (
                                                            ...
                                                            {student.paymentMode === 'Cheque' ? (
                                                                student.paymentStatus === 'Paid' ? (
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        onChange={(e) => handleChequeFieldChange(student._id, 'bankName', e.target.value)}
                                                                        required
                                                                    />
                                                                )
                                                            ) : (
                                                ...
                                                <td>
                                                    <span className={`badge ${student.paymentStatus === 'Paid' ? 'bg-success' : 'bg-warning text-dark'}`}>
