...
const CreateFeesRefund = () => {
  ...
  const [errorFeeType, setErrorFeeType] = useState(null); 
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    // Determine installmentName for School Fees
    let installmentName = null;
    if (selectedRefundType === 'School Fees') {
      // Get the installmentName from the fee records for the selected student
      const relevantFeeRecords = filteredStudents.filter(
        (fee) =>
          fee.feeType.trim() === 'School Fees' &&
          fee.admissionNumber === selectedStudent.admissionNumber &&
          feeTypeRefundAmounts[fee.feetype] &&
          parseFloat(feeTypeRefundAmounts[fee.feetype]) > 0
      );

      // Use the first matching installmentName (assuming all selected fee types belong to the same installment)
      if (relevantFeeRecords.length > 0) {
        installmentName = relevantFeeRecords[0].installmentName || null;
      }
    }
    ...
    const refundData = {
      ...
      feeTypeRefunds: selectedRefundType === 'School Fees'
        ...
        : [],
      installmentName, // Add installmentName to refundData
    };
