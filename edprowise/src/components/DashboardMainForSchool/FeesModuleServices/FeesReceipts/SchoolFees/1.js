...
const SchoolFeesReceipt = () => {
  ...
  const [student, setStudent] = useState(receiptDetails[0] || {}); 
  const [isCancelledOrReturned, setIsCancelledOrReturned] = useState(
    ["Cancelled", "Cheque Return"].includes(receiptDetails?.[0]?.status) ||
      receiptDetails?.[0]?.reportStatus?.some((status) =>
  ...
  useEffect(() => {
    if (
      ...
      const classData = classes.find((cls) => cls._id === receiptDetails[0].className);
      if (classData) {
        setClassName(classData.className || "Unknown Class");
        const sectionData = classData.sections?.find((sec) => sec._id === receiptDetails[0].section);
  ...
  return (
    <div className="container my-4" style={{ maxWidth: "800px" }}>
      ...
      <CancelReceiptModal
        ...
        feeTypeName={feeTypes.find((type) => type._id === student?.installments?.[0]?.feeItems?.[0]?.feeTypeId)?.feesTypeName || "School Fee"}
