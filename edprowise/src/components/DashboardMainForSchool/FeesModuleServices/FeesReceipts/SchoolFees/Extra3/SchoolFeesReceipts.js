              ...
              <form>
                <>
                  ...
                  <div className="table-responsive mt-3">
                    ...
                    <table
                      className="table align-middle mb-0  table-centered text-center text-nowrap"
                    >
                      <thead>
                                ...
                                return year.installmentsPresent.map((installmentNum) => {
                                  ...
                                  const totals = installmentData.reduce(
                                    (acc, item) => {
                                      ...
                                      const finePaidKey = `${year.academicYear}-${installmentNum}-fine`;
                                      const excessPaidKey = `${year.academicYear}-${installmentNum}-excess`;
                                      const currentPaidAmount = paidAmounts[paidKey] !== undefined
                                        ? Number(paidAmounts[paidKey] || 0)
                                        : 0;
                                      const finePaidAmount = paidAmounts[finePaidKey] !== undefined
                                        ? Number(paidAmounts[finePaidKey] || 0)
                                        : 0;
                                      // Ensure excessPaidAmount is only counted once per installment
                                      const excessPaidAmount = paidAmounts[excessPaidKey] !== undefined
                                        ? Number(paidAmounts[excessPaidKey] || 0)
                                        : 0;

                                      const balance = item.balanceAmount + fineAmount;
                                      const amountPaid = payableAmount - item.balanceAmount - fineAmount;
                                      ...
                                      return {
                                        ...
                                        totalPaid: acc.totalPaid + (currentPaidAmount + finePaidAmount + (acc.excessPaidProcessed ? 0 : excessPaidAmount)), // Prevent double-counting excess
                                        totalBalance: acc.totalBalance + balance,
                                        totalPaidAmount: acc.totalPaidAmount + amountPaid,
                                        excessPaidAmount: acc.excessPaidAmount + excessPaidAmount, // Accumulate excessPaidAmount
                                        excessPaidProcessed: true, // Flag to ensure excess is only counted once
                                      };
                                    },
                                    {
                                      totalFeesAmount: 0,
                                  ...
                                  return (
                                    <React.Fragment key={`${year.academicYear}-${installmentNum}`}>
                                      <tr >
                                        ...
                                        <td>{formatAcademicYear(year.academicYear)}</td>
                                        <td>{installmentData[0].installmentName}</td>
                                        ...
                                        <td>
                                          <input
                                            ...
                                            value={totals.totalFine || 0}
                                            onChange={(e) => {
                                              const newFineAmount = e.target.value;
                                              installmentData.forEach((item) => {
                                                handleFineAmountChange(
                                                  installmentNum,
                                                  item.feesTypeId._id,
                                                  newFineAmount / installmentData.length,
                                                  year.academicYear
                                                );
                                              });
                                            }}
                                            min="0"
                                            style={{ width: '60px', border: '1px solid #f2be00', textAlign: 'right', fontSize: '1.2rem' }}
                                          />
                                        </td>
                                      ...
                                      {showReceiptsTable && year.paidInstallments && year.paidInstallments.length > 0 && (() => {
                                        ...
                                        const groupedByReceipt = filteredInstallments.reduce((acc, item) => {
                                          ...
                                          const concessionItem = year.concession?.concessionDetails?.find(
                                            (cd) =>
                                              cd.installmentName === cd.installmentName &&
                                              cd.feesType === item.feesTypeId?._id
                                          );
                                          const concessionAmount = concessionItem?.concessionAmount || 0;
                                          const fineAmount = item.fineAmount || 0;
                                                ...
                                                <table className="table table-bordered table-sm">
                                                  ...
                                                  <tbody>
                                                      ...
                                                      return (
                                                        <React.Fragment key={`${year.academicYear}-${installmentNum}-paid-${receiptNumber}`}>
                                                          {group.items.map((item, itemIndex) => (
                                                            <tr 
                                                            key={`${year.academicYear}-${installmentNum}-paid-${receiptNumber}-${itemIndex}`}>
                                                              {itemIndex === 0 && (
                                                    ...
                                                    {sortedReceiptNumbers.length > 0 && (
                                                      <tr style={{borderTop:"black"}} className="fw-bold bg-light">
                                  ...
                                  .reduce((sum, year) => {
                                    const yearTotal = year.feeInstallments
                                      ...
                                      ?.reduce((acc, item) => {
                                        ...
                                        const finePaidKey = `${year.academicYear}-${item.installmentName.split(' ')[1]}-fine`;
                                        const excessPaidKey = `${year.academicYear}-${item.installmentName.split(' ')[1]}-excess`;
                                        let currentPaidAmount = paidAmounts[paidKey] !== undefined ? Number(paidAmounts[paidKey] || 0) : 0;
                                        currentPaidAmount += paidAmounts[finePaidKey] !== undefined ? Number(paidAmounts[finePaidKey] || 0) : 0;
                                        currentPaidAmount += paidAmounts[excessPaidKey] !== undefined ? Number(paidAmounts[excessPaidKey] || 0) : 0;
                ...
                {showSecondTable && (
                  <div className="text-end my-3">
                    {!showProcessedData ? (
                      ...
                      <>
                        <div className="mt-4">
                          <h4 className="card-title text-start">Payment Details</h4>
                          <div className="row">
                            <div className="col-md-6">
