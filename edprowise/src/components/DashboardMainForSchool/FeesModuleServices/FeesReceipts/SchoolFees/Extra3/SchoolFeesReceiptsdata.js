...
const openFeeTypeModal = (installmentNum, academicYear) => {
  ...
  const totalFineAmount = feeItems.reduce((sum, item) => sum + (item.fineAmount || 0), 0);
  const finePaidKey = `${academicYear}-${installmentNum}-fine`;
  const totalFinePaid = totalFineAmount > 0 && paidAmounts[finePaidKey] !== undefined ? Number(paidAmounts[finePaidKey] || 0) : 0;

  const excessKey = `${academicYear}-${installmentNum}-excess`;
  ...
  setPaidAmounts((prev) => {
    ...
    feeItems.forEach((item) => {
      if (item.paidAmount !== '') {
        newPaidAmounts[`${academicYear}-${installmentNum}-${item.feeTypeId}`] = item.paidAmount;
      }
    });


    if (totalFinePaid > 0) {
      newPaidAmounts[finePaidKey] = totalFinePaid;
    }

 
    newPaidAmounts[excessKey] = excessAmount;

    console.log('openFeeTypeModal: paidAmounts:', newPaidAmounts);
 ...
 const handleModalPaidAmountChange = (installmentNum, feeTypeId, amount, academicYear) => {
  ...
  setPaidAmounts((prev) => {
    ...
    if (parsedAmount === '' || parsedAmount === 0) {
      ...
      console.log(`Cleared paidAmount for ${key}`);
    } else {
      newPaidAmounts[key] = parsedAmount;
      console.log(`Set paidAmount for ${key} to ${parsedAmount}`);
    }
    console.log('handleModalPaidAmountChange: paidAmounts:', newPaidAmounts);
    return newPaidAmounts;
  });
    ...
    setSelectedInstallments((prev) => {
      ...
      if (isSelected) {
        ...
        installmentData.forEach((item) => {
          ...
          const fineKey = `${academicYear}-${installmentNumber}-fine`;
          const excessKey = `${academicYear}-${installmentNumber}-excess`;
          delete newPaidAmounts[key];
  ...
  const handleSelectAllInstallments = () => {
    ...
    setSelectedInstallments((prev) => {
      ...
      if (!newSelectAll) {
        ...
        actualSelectedYears.forEach((academicYear) => {
          ...
          if (yearData && Array.isArray(yearData.installmentsPresent)) {
            yearData.installmentsPresent.forEach((installmentNum) => {
              ...
              installmentData.forEach((item) => {
                const key = `${academicYear}-${installmentNum}-${item.feesTypeId._id}`;
                const fineKey = `${academicYear}-${installmentNum}-fine`;
                const excessKey = `${academicYear}-${installmentNum}-excess`;
                delete newPaidAmounts[key];
                delete newPaidAmounts[fineKey];
                delete newPaidAmounts[excessKey];
        ...
        console.log('Cleared paidAmounts after deselecting all installments:', newPaidAmounts);
        setPaidAmounts(newPaidAmounts);
      }
    ...
    setSelectedFeeTypesByInstallment((prev) => {
      ...
      if (!isAlreadySelected) {
        ...
        setPaidAmounts((prev) => {
          ...
          console.log('Cleared paidAmounts after deselecting fee type:', newPaidAmounts); 
          return newPaidAmounts;
        });
      }
  ...
  setPaidAmounts((prev) => {
    ...
    if (action === 'Full Fees') {
      ...
      installmentData.forEach((item) => {
        ...
        const paidKey = `${academicYear}-${installmentNumber}-${item.feesTypeId._id}`;
        const finePaidKey = `${academicYear}-${installmentNumber}-fine`;
 ...
 const handleFineAmountChange = (installmentNum, feeTypeId, newFineAmount, academicYear) => {
  ...
  setPaidAmounts((prev) => {
    ...
    if (parsedFineAmount === 0) {
      delete newPaidAmounts[finePaidKey]; 
      console.log(`Cleared fine for ${finePaidKey}, newFineAmount: ${newFineAmount}`);
    } else {
      newPaidAmounts[finePaidKey] = parsedFineAmount;
      console.log(`Set fine for ${finePaidKey} to ${parsedFineAmount}`);
    }
    ...
    return prev.map((year) => {
      if (year.academicYear === academicYear) {
        const updatedInstallments = year.feeInstallments.map((item) => {
          if (
            ...
            return {
              ...
              // balanceAmount: payableAmount + parsedFineAmount - currentPaidAmount,
            };
          }
          return item;
        });
        ...
        const totalFeesPayable = updatedInstallments.reduce((sum, item) => {
          ...
          return sum + (item.amount - concession + (item.fineAmount || 0));
        }, 0);
        const totalPaid = updatedInstallments.reduce((sum, item) => {
          ...
          const finePaidKey = `${academicYear}-${item.installmentName.split(' ')[1]}-fine`;
          const excessPaidKey = `${academicYear}-${item.installmentName.split(' ')[1]}-excess`;
          let paid = paidAmounts[paidKey] !== undefined ? Number(paidAmounts[paidKey] || 0) : 0;
          paid += paidAmounts[finePaidKey] !== undefined ? Number(paidAmounts[finePaidKey] || 0) : 0;
          paid += paidAmounts[excessPaidKey] !== undefined ? Number(paidAmounts[excessPaidKey] || 0) : 0;
          return sum + paid;
        }, 0);
    ...
    try {
      ...
      for (const academicYear of actualSelectedYears) {
        ...
        for (const installmentNum of selectedInstallments[academicYear] || []) {
          ...
          const uniqueFeeItems = installmentData
            ...
            .map((item) => {
              ...
              const fineAmount = item.fineAmount || 0;
              const payable = item.balanceAmount - concession;
              const paidKey = `${item.academicYear}-${installmentNum}-${item.feesTypeId._id}`;
              const finePaidKey = `${item.academicYear}-${installmentNum}-fine`;
              const paid = Number(paidAmounts[paidKey] || 0);
              const finePaid = Number(paidAmounts[finePaidKey] || 0);
              return [
                {
                  feeTypeId: item.feesTypeId._id,
                  amount: item.amount,
          ...
          if (uniqueFeeItems.length > 0 || excessAmount > 0) {
            receiptDetails.installments.push({
              number: parseInt(installmentNum),
              excessAmount,
        ...
        if (receiptDetails.installments.length === 0) {
          console.warn(`No valid fee items or excess amount for academic year ${academicYear}`);
          continue;
        }
        ...
        frontendReceiptDetails.push({
          ...
          installments: receiptDetails.installments.map((inst) => ({
            ...
            feeItems: inst.feeItems.map((item) => ({
              ...item,
              type: item.feeTypeId === 'fine' ? 'Fine' : getFeeTypeName(item.feeTypeId),
              academicYear,
            })),
          })),
        });
      ...
      if (frontendReceiptDetails.length === 0) {
        throw new Error('No valid fee items or excess amount selected for any academic year');
      }
