...
const useSchoolFeesReceipts = () => {
  ...
  const feeItems = installmentData
    .filter((item) => {
      if (seenFeeTypeIds.has(item.feesTypeId._id)) {
        console.warn(`Duplicate feeTypeId ${item.feesTypeId._id} found for ${academicYear}-${installmentName}`);
        return false;
      }
 ...
 const handleActionSelection = (installmentName, academicYear, action) => {
  ...
  setPaidAmounts((prev) => {
    ...
    // Clear existing paid amounts for fee types, but preserve fine and excess if they exist
    installmentData.forEach((item) => {
      const paidKey = `${academicYear}-${installmentName}-${item.feesTypeId._id}`;
      delete newPaidAmounts[paidKey]; // Clear only fee type amounts
    });

    // Preserve fine and excess amounts unless explicitly resetting
    const fineKey = `${academicYear}-${installmentName}-fine`;
    const excessKey = `${academicYear}-${installmentName}-excess`;
    ...
    if (action === 'Full Fees') {
      ...
      // Set paid amounts for full fees
      installmentData.forEach((item) => {
        const concessionItem = feeData
          .find((y) => y.academicYear === academicYear)
          ?.concession?.concessionDetails?.find(
        ...
        if (fineAmount > 0) {
          newPaidAmounts[finePaidKey] = fineAmount; // Set fine amount for full fees
        }
      });
    } else if (action === 'Part Fees') {
      setSelectedFeeTypesByInstallment((prev) => {
        const yearTypes = prev[academicYear] || {};
        const currentTypes = yearTypes[installmentName] || [];
        if (currentTypes.length === 0) {
          installmentData.forEach((item) => {
            const paidKey = `${academicYear}-${installmentName}-${item.feesTypeId._id}`;
            newPaidAmounts[paidKey] = ''; // Initialize fee type amounts to empty
          });
          // Do not reset finePaidKey here to preserve existing fine amount
          return {
            ...prev,
            [academicYear]: {
              ...yearTypes,
      ...
      // Clear selections for no action
      setSelectedFeeTypesByInstallment((prev) => {
        const newTypes = { ...prev };
        if (newTypes[academicYear]) {
          delete newTypes[academicYear][installmentName];
      ...
      // Clear all paid amounts, including fine and excess, when action is cleared
      installmentData.forEach((item) => {
        const paidKey = `${academicYear}-${installmentName}-${item.feesTypeId._id}`;
        const fineKey = `${academicYear}-${installmentName}-fine`;
        const excessKey = `${academicYear}-${installmentName}-excess`;
    ...
    // Update modal data to reflect preserved fine amount
    setModalData((prev) => {
      ...
      const updatedFeeItems = prev.feeItems.map((item) => {
        if (item.isFine) {
          ...
          return {
            ...item,
            amount: fineAmount,
            paidAmount: newPaidAmounts[fineKey] || 0, // Preserve fine paid amount
        ...
        for (const installmentName of selectedInstallments[academicYear] || []) {
          ...
          const uniqueFeeItems = installmentData
            ...
            .map((item) => {
              ...
              const payable = item.amount - concession + (item.fineAmount || 0);
