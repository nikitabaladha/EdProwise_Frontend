  ...
  const handleAdmissionSubmit = async (e) => {
    ...
    try {
      const response = await getAPI(
        `/get-schoolfees?classId=${student.masterDefineClass}&sectionIds=${student.section}&schoolId=${schoolId}&admissionNumber=${formData.AdmissionNumber}`
      );
...
const handleProceed = () => {
  ...
  const receiptDetails = concessionData
    ...
    .map((data) => {
      ...
      return {
        receiptNumber: data.paymentInfo?.receiptNumber || "N/A",
        studentName: `${selectedStudent.firstName} ${selectedStudent.lastName}`,
        studentAdmissionNumber: selectedStudent.AdmissionNumber,
        paymentDate: new Date(data.paymentInfo?.paymentDate || Date.now()).toLocaleDateString(),
        date: new Date(data.paymentInfo?.createdAt || Date.now()).toLocaleDateString(),
        academicYear: data.academicYear,
        className: classInfo?.className || "Unknown",
        section: sectionInfo?.name || "Unknown",
        paymentMode: data.paymentInfo?.paymentMode || "N/A",
        transactionNumber: data.paymentInfo?.transactionNumber || "N/A",
        bankName: data.paymentInfo?.bankName || "N/A",
        chequeNumber: data.paymentInfo?.chequeNumber || "N/A",
        collectorName: data.paymentInfo?.collectorName || "School Administrator",
        installments,
      };
    })
