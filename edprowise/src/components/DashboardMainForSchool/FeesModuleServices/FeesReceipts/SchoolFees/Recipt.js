        ...
        {receiptDetails.map((receipt, receiptIndex) =>
          receipt.installments.map((installment, instIndex) => (
            ...
            >
                  ...
                  <tbody>
                    {installment.feeItems.map((item, itemIndex) => (
                      <tr key={itemIndex}>
                        ...
                        <td className="text-center text-danger">{item.concession}</td>
                    ...
                    <tr className="table-secondary fw-bold">
                      ...
                      <td className="text-center">
                        {installment.feeItems.reduce((sum, item) => sum + Number(item.amount), 0)}
                      </td>
                      <td className="text-center text-danger">
                        {installment.feeItems.reduce((sum, item) => sum + Number(item.concession), 0)}
                      </td>
                      <td className="text-center">
                        {installment.feeItems.reduce((sum, item) => sum + Number(item.fineAmount), 0)}
                      </td>
                      <td className="text-center">
                        {installment.feeItems.reduce((sum, item) => sum + Number(item.payable), 0)}
                      </td>
                      <td className="text-center">
                        {installment.feeItems.reduce((sum, item) => sum + Number(item.paid), 0)}
                      </td>
                      <td className="text-center">
                        {installment.feeItems.reduce((sum, item) => sum + Number(item.balance), 0)}
                      </td>
                    </tr>
                  </tbody>
                </table>
              ...
              {/* Footer */}
              <div className="row mb-2 text-black" style={{ pageBreakInside: "avoid", breakInside: "avoid" }}>
                <div className="col-md-6">
                  <div className="d-flex mb-2">
                    <span className="fw-bold me-2" style={{ minWidth: "150px" }}>
                      Payment Mode:
                    </span>
                    <span className="text-capitalize">{receipt.paymentMode}</span>
                  </div>
