...
const SchoolFeesReceipt = () => {
    ...
    return `
      ...
      <div class="pdf-container">
        ...
        ? `
              ...
              `
        : ""}
          ${['Cancelled', 'Cheque Return'].includes(student?.[0]?.status) ? `
            <div class="pdf-field"><span class="pdf-label">Cancel Reason:</span><span class="pdf-value">${student?.[0]?.cancelReason || "N/A"}</span></div>
          ` : ""}
          </div>
          <div class="pdf-col-6">
  ...
  const handleActionSelect = (action) => {
    if (isCancelledOrReturned) {
      toast.info(`Receipt is already ${student?.[0]?.status.toLowerCase()}.`, {
