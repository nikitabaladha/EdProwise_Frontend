      ...
      try {
        ...
        reader.onload = async (e) => {
          ...
          for (let index = 0; index < jsonData.length; index++) {
            ...
            payload.installmentNumber = row.installmentNumber?.toString().trim();
            ...
            if (!student) {
              toast.error(`Row ${index + 2}: Invalid Admission Number "${payload.AdmissionNumber}".`);
              hasError = true;
              continue;
            }
            payload.firstName = student.firstName;
            payload.lastName = student.lastName;
            payload.masterDefineClass = student.masterDefineClass?._id || student.masterDefineClass;
            ...
            if (!sectionObj) {
              toast.error(`Row ${index + 2}: Invalid Section "${sectionName}" for class "${className}".`);
              hasError = true;
              continue;
            }
            ...
            if (isNaN(installmentNum) || installmentNum < 1) {
              toast.error(`Row ${index + 2}: Invalid Installment Number "${payload.installmentNumber}".`);
              hasError = true;
              continue;
            }
            ...
            const feeType = feeTypes.find(
              (ft) => ft.feesTypeName.toLowerCase() === payload.feeTypeName?.toLowerCase()
            );
            if (!feeType) {
              toast.error(`Row ${index + 2}: Invalid Fee Type "${payload.feeTypeName}".`);
              hasError = true;
              continue;
            }
            ...
            if (isNaN(parsedDate.getTime())) {
              toast.error(`Row ${index + 2}: Invalid Payment Date "${payload.paymentDate}". Use format YYYY-MM-DD.`);
              hasError = true;
              continue;
            }
            ...
            if (!payload.masterDefineClass) {
              toast.error(`Row ${index + 2}: Class ID is missing for Admission Number "${payload.AdmissionNumber}".`);
              hasError = true;
              continue;
            }
            if (!payload.section) {
              toast.error(`Row ${index + 2}: Section ID is missing for Admission Number "${payload.AdmissionNumber}".`);
              hasError = true;
              continue;
            }
            if (!schoolId) {
              toast.error(`Row ${index + 2}: School ID is missing. Please contact support.`);
              hasError = true;
              continue;
            }
            ...
            try {
              ...
              if (!response?.data?.data || !Array.isArray(response.data.data)) {
                toast.error(`Row ${index + 2}: No fee data found for Admission Number "${payload.AdmissionNumber}".`);
                hasError = true;
                continue;
              }
              ...
              if (!installmentData) {
                toast.error(
                  `Row ${index + 2}: Installment ${installmentNum} with Fee Type "${payload.feeTypeName}" not found for Academic Year "${payload.academicYear}".`
                );
                hasError = true;
                continue;
              }
              ...
              if (payload.paidAmount <= 0 || payload.paidAmount > payableAmount) {
                toast.error(
                  `Row ${index + 2}: Paid Amount (${payload.paidAmount}) must be between 1 and ${payableAmount}.`
                );
                hasError = true;
                continue;
              }
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      const groupedData = validatedData.reduce((acc, data) => {
        ...
        if (!installment) {
          installment = { number: installmentNum, academicYear: data.academicYear, feeItems: [] };
      ...
      for (const data of Object.values(groupedData)) {
        ...
        try {
          const receiptDetails = {
            ...
            section: sectionsByClass[data.masterDefineClass]?.find((s) => s._id === data.section)?.name || 'Unknown',
            paymentDate: data.paymentDate, 
            paymentMode: data.paymentMode,
          ...
          toast.success(`School Fees Data for ${data.firstName} ${data.lastName} imported successfully.`);
        } catch (error) {
          toast.error(
            `Failed to import for Admission Number ${data.AdmissionNumber}: ${error.message || 'Unknown error'}`
          );
          continue;
        }
      }
