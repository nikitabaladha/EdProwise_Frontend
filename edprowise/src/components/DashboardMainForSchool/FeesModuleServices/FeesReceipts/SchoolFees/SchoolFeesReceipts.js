...
//             academicYear: year.academicYear,
//             installmentName,
//             dueDate: new Date(installmentData[0].dueDate), // Store as Date object for sorting
//             totals,
//             isInstallmentSelected,
//           };
//         }).filter(Boolean);
//       })
//       .sort((a, b) => a.dueDate - b.dueDate) // Sort by dueDate
//       .map(({ academicYear, installmentName, dueDate, totals, isInstallmentSelected }) => (
//         <tr key={`${academicYear}-${installmentName}`}>
...
//                 onChange={() => handleInstallmentCheckbox(installmentName, academicYear)}
//               />
//             ) : (
...
//           <td><b>{totals.totalFeesAmount}</b></td>
//           <td><b>{totals.totalConcession}</b></td>
//           <td><b>{totals.totalPayable}</b></td>
//           <td><b>{totals.totalPaidAmount}</b></td>
//           <td><b>{totals.totalBalance}</b></td>
//           <td><b>{totals.totalPaid}</b></td>
//           {totals.totalBalance > 0 ? (
//             <td>
//               <select
//                 style={{ width: '120px', border: '1px solid #f2be00', fontSize: '1.0rem' }}
//                 className="form-select form-control-sm"
//                 value={actionSelections[`${academicYear}-${installmentName}`] || ''}
//                 onChange={(e) =>
//                   handleActionSelection(installmentName, academicYear, e.target.value)
//                 }
//               >
...
//                 onClick={() => openFeeTypeModal(installmentName, academicYear)}
//               >
//                 <FaEye />
//               </button>
//             </td>
...
//               type="button"
//               className="btn btn-sm btn-primary"
//               onClick={() => toggleReceiptsTable(academicYear, installmentName)}
//               aria-label={visibleReceipts[`${academicYear}-${installmentName}`] ? `Hide receipts for ${installmentName}` : `View receipts for ${installmentName}`}
//             >
//               <FaEye />
//             </button>
//           </td>
...
const toggleReceiptsTable = async (academicYear, installmentName) => {
  ...
  const renderReceiptsTable = () => {
    ...
    const receiptsToShow = feeData
      .filter((year) => {
        if (!year?.academicYear) {
          console.warn('Invalid year object:', year);
          return false;
        }
        return selectAllYears || selectedAcademicYears.includes(year.academicYear);
      })
      .flatMap((year) => {
        if (!Array.isArray(year.installmentsPresent) || year.installmentsPresent.length === 0) {
          console.warn(`No installmentsPresent for academicYear ${year.academicYear}`);
          return [];
        }
        ...
        return year.installmentsPresent
          .filter((installmentName) => {
            const key = `${year.academicYear}-${installmentName}`;
            const isVisible = visibleReceipts[key];
            return isVisible && installmentName;
          })
          .map((installmentName) => {
            const key = `${year.academicYear}-${installmentName}`;
            let receipts = Array.isArray(receiptsData[key]) ? receiptsData[key] : [];
            if (receiptsData[key] && receiptsData[key].data && Array.isArray(receiptsData[key].data)) {
              receipts = receiptsData[key].data;
            }
            ...
            const groupedByReceipt = receipts.reduce((acc, item) => {
              const receiptNumber = item.receiptNumber?.toString() || '';
              if (!receiptNumber) {
                return acc;
              }
              ...
              (item.feeItems || []).forEach((feeItem) => {
                const concessionAmount = Number(feeItem.concession) || 0;
                const feesAmount = Number(feeItem.amount) || 0;
                const payableAmount = Number(feeItem.payable) || feesAmount - concessionAmount;
                const paidAmount =
                  item.status === 'Cancelled' || item.status === 'Cheque Return'
                    ? Number(feeItem.cancelledPaidAmount) || Number(feeItem.paid) || 0
                    : Number(feeItem.paid) || 0;
                const balance = Number(feeItem.balance) || 0;

                totals.totalFeesAmount += feesAmount;
                totals.totalConcession += concessionAmount;
                totals.totalPayable += payableAmount;
                totals.totalPaid += paidAmount;
                totals.totalBalance += balance;
                totals.totalFine += Number(item.fineAmount) || 0;
                totals.totalExcess += Number(item.excessAmount) || 0;

                const feesType = getFeeTypeName(feeItem.feeTypeId) || 'Unknown Fee Type';
                const existingItem = acc[receiptNumber].items.find((i) => i.feesType === feesType);
                if (!existingItem) {
                  acc[receiptNumber].items.push({
                    feesType,
                    feesAmount,
            ...
            const rows = sortedReceiptNumbers.map((receiptNumber, rowIndex) => {
              const group = groupedByReceipt[receiptNumber];
              if (!group || !group.items) {
                console.warn('Invalid group for receiptNumber:', receiptNumber);
                return null;
              }
    ...
    if (receiptsToShow.length === 0) {
      console.warn('No receipts to show after filtering');
      return (
        <div className="mt-4 text-center">
          ...
          <div key={`${academicYear}-${installmentName}-${index}`} className="mt-3">
            ...
            <table className="table table-bordered table-sm">
              ...
              <tbody>
                {rows.length > 0 && !rows[0].isEmpty ? (
                  rows.map((row, rowIndex) => (
                    <tr key={`${academicYear}-${installmentName}-receipt-${row.receiptNumber}-${rowIndex}`}>
                      <td>{row.receiptNumber}</td>
                      <td>{row.paymentDate}</td>
                      <td>{row.collectorName}</td>
                      <td>{row.paymentMode}</td>
                              ...
                              .flatMap((year) => {
                                ...
                                return year.installmentsPresent.map((installmentName) => {
                                  ...
                                  return {
                                    academicYear: year.academicYear,
                                    installmentName,
                                    dueDate: installmentData?.[0]?.dueDate ? new Date(installmentData[0].dueDate) : new Date(),
                                    totals,
                                    isInstallmentSelected,
                                  };
                                }).filter(Boolean);
