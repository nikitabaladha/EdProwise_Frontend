...
import CancelReceiptModal from "../../CancelReceiptModalSchoolFees";

const SchoolFeesReceipt = () => {
  ...
  useEffect(() => {
    if (!schoolId || !receiptNumber) return;

    const fetchInitialData = async () => {
      try {
        const schoolInfo = await fetchSchoolData(schoolId);
        setSchoolData(schoolInfo);
        const receipts = receiptNumber.map(item => item.receiptNumber);

 
        const receiptResponse = await getAPI(`/get-school-fees-data/${schoolId}/${receipts}`, {}, true);
        if (!receiptResponse.hasError && receiptResponse.data) {
          const data = receiptResponse.data;
          setReceiptDetails([data]);
          setStudent(data);
          setClassName(data.className || "Unknown Class");
          setSectionName(data.sectionName || "Unknown Section");
          setIsCancelledOrReturned(
            ["Cancelled", "Cheque Return"].includes(data.status) ||
            data.reportStatus?.some((status) =>
              ["Refund", "Cancelled", "Cheque Return"].includes(status)
            )
          );
          const feeInstallmentsResponse = await getAPI(
            `/get-schoolfees?classId=${data.classId}&sectionIds=${data.sectionId}&schoolId=${schoolId}&admissionNumber=${data.studentAdmissionNumber}&academicYear=${data.academicYear}`
          );
          if (
            !feeInstallmentsResponse?.data?.data ||
            !Array.isArray(feeInstallmentsResponse.data.data) ||
            !feeInstallmentsResponse.data.data[0]?.feeInstallments
          ) {
            toast.error("Failed to fetch fee installments data");
            return;
          }
          setFeeInstallments(feeInstallmentsResponse.data.data[0].feeInstallments);
        } else {
  ...
  const handleModalClose = async (updatedStudent) => {
    ...
    if (updatedStudent) {
      ...
      setIsCancelledOrReturned(["Cancelled", "Cheque Return", "Refund"].includes(updatedStudent.status));
    }
  };
    ...
    return `
      <style>
        * {
        ...
        .pdf-table-secondary th:last-child, .pdf-table-secondary td:last-child {
          text-align: center;
        }
        .pdf-signature {
          text-align: center;
      ...
      <div class="pdf-container">
        ...
        <div class="pdf-row">
          <div class="pdf-col-6">
            ...
            <div class="pdf-field"><span class="pdf-label">Student Name:</span><span class="pdf-value">${receipt.firstName}${receipt.lastName}</span></div>
          ...
          <div class="pdf-col-6">
            <div class="pdf-field"><span class="pdf-label">Date:</span><span class="pdf-value">${receipt.paymentDate ? new Date(receipt.paymentDate).toLocaleDateString("en-GB") : "N/A"}</span></div>
            ...
            ${(() => {
              ...
              return Array.from({ length: rowCount })
                .map(
                  (_, i) => `
                    <tr>
                      <td>${leftItems[i]?.feeTypeName || leftItems[i]?.type || ""}</td>
                      <td>${leftItems[i]?.paid || ""}</td>
                      <td>${rightItems[i]?.feeTypeName || rightItems[i]?.type || ""}</td>
        ...
        <div class="pdf-table-heading">Balance Payable Exclude Fine & Excess Fees</div>
        <table class="pdf-table pdf-table-secondary">
          <thead>
            <tr>
              <th>Type of Fees</th>
              <th>Fees</th>
            ...
            ${feeInstallments.length > 0
              ? feeInstallments
                  ...
                  .map(
                    (fee) => `
                    <tr>
                      <td>${feeTypes.find((type) => type._id === fee.feesTypeId?._id)?.feesTypeName || `Fee ID ${fee.feesTypeId || "Unknown"}`}</td>
                      <td>${fee.amount || 0}</td>
                  ...
                  .join("")
              : `<tr><td colspan="6" style="text-align: center; color: #6b7280;">${feeInstallments === null ? "Loading fee installment data..." : "No fee installment data available"}</td></tr>`}
            <tr class="pdf-table-secondary">
              <td><strong>Total</strong></td>
              <td><strong>${totals.totalFeesAmount || 0}</strong></td>
              <td><strong>${totals.totalConcession || 0}</strong></td>
              <td><strong>${totals.totalFeesPayable || 0}</strong></td>
              <td><strong>${totals.totalPaidAmount || 0}</strong></td>
              <td><strong>${totals.totalRemainingAmount || 0}</strong></td>
            </tr>
          </tbody>
        </table>
        <div class="pdf-row">
  ...
  if (!receiptNumber || !receiptDetails.length) {
    return <div className="container my-4">No receipt data found</div>;
  }

  const handleActionSelect = (action) => {
    if (isCancelledOrReturned) {
      toast.info(`Receipt is already ${student?.status?.toLowerCase() || "processed"}.`, {
          ...
          receipt.installments.map((installment, instIndex) => {
            ...
            return (
              ...
              >
                      ...
                      <tbody>
                        {(() => {
                          ...
                          return Array.from({ length: rowCount }).map((_, i) => (
                            <tr key={i}>
                              <td className="text-center">{leftItems[i]?.feeTypeName || leftItems[i]?.type || ""}</td>
                              <td className="text-center">{leftItems[i]?.paid || ""}</td>
                              <td className="text-center">{rightItems[i]?.feeTypeName || rightItems[i]?.type || ""}</td>
                ...
                >
                  <h4 className="mb-3 fw-bold">Balance Payable Exclude Fine & Excess Fees</h4>
                  <div className="table-responsive mb-3">
                    <table className="table table-bordered">
                      <thead className="table-primary">
                        <tr>
                          <th className="text-left">Type of Fees</th>
                          <th className="text-left">Fees</th>
                          <th className="text-left">Concession</th>
                      ...
                      <tbody>
                          ...
                          feeInstallments
                            ...
                            .map((fee, itemIndex) => (
                              <tr key={itemIndex}>
                                ...
                                <td className="text-left">{fee.amount || 0}</td>
                                <td className="text-left">{fee.concessionAmount || 0}</td>
                                <td className="text-left">
                                  {(fee.amount || 0) - (fee.concessionAmount || 0)}
                                </td>
                                <td className="text-left">{fee.paidAmount || 0}</td>
                        ...
                        <tr className="table-secondary fw-bold">
                          ...
                          <td className="text-left"><strong>{totals.totalFeesAmount || 0}</strong></td>
                          <td className="text-left"><strong>{totals.totalConcession || 0}</strong></td>
                          <td className="text-left"><strong>{totals.totalFeesPayable || 0}</strong></td>
                          <td className="text-left"><strong>{totals.totalPaidAmount || 0}</strong></td>
                          <td className="text-center"><strong>{totals.totalRemainingAmount || 0}</strong></td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
...
export default SchoolFeesReceipt;