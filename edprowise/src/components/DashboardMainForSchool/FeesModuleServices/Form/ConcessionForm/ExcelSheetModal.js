...
const ExcelSheetModal = ({ show, onClose, schoolId, academicYear, onImportSuccess, classes }) => {
      ...
      try {
        ...
        reader.onload = async (e) => {
          ...
          jsonData.forEach((row, index) => {
            ...
            groupedData[admissionNumber].rows.push({ ...row, rowIndex: index + 1 });
          });
          ...
          for (const admissionNumber in groupedData) {
            ...
            if (!student) {
              toast.error(`Row ${rows[0].rowIndex}: Invalid Admission Number "${admissionNumber}".`);
              hasError = true;
              rowValid = false;
            } else {
              payload.firstName = rows[0].firstName?.toString().trim() || student.firstName || '';
              payload.middleName = rows[0].middleName?.toString().trim() || student.middleName || '';
              payload.lastName = rows[0].lastName?.toString().trim() || student.lastName || '';
              payload.concessionType = rows[0].concessionType?.toString().trim() || '';
              console.log(`Row ${rows[0].rowIndex}: Found student for AdmissionNumber ${admissionNumber}`);
            }
            ...
            if (!classObj) {
              toast.error(`Row ${rows[0].rowIndex}: Invalid Class "${className}".`);
              hasError = true;
              rowValid = false;
            } else {
              payload.masterDefineClass = classObj._id;
              console.log(`Row ${rows[0].rowIndex}: Class ${className} mapped to ID ${classObj._id}`);
            }
            ...
            if (!sectionObj) {
              toast.error(`Row ${rows[0].rowIndex}: Invalid Section "${sectionName}" for class "${className}".`);
              hasError = true;
              rowValid = false;
            } else {
              payload.section = sectionObj._id;
              console.log(`Row ${rows[0].rowIndex}: Section ${sectionName} mapped to ID ${sectionObj._id}`);
            }

            const validConcessionTypes = ['EWS', 'SC', 'ST', 'OBC', 'Staff Children', 'Other'];
            if (!validConcessionTypes.includes(payload.concessionType)) {
              toast.error(`Row ${rows[0].rowIndex}: Invalid Concession Type "${payload.concessionType}". Must be one of: ${validConcessionTypes.join(', ')}.`);
              hasError = true;
              rowValid = false;
            }
            ...
            const requiredFields = [
              ...
              { key: 'concessionType', value: payload.concessionType, label: 'Concession Type' },
            ];

            for (const field of requiredFields) {
              if (!field.value) {
                toast.error(`Row ${rows[0].rowIndex}: ${field.label} is required.`);
                hasError = true;
                rowValid = false;
              }
            }
            ...
            rows.forEach((row, rowIndex) => {
              ...
              if (!feeType) {
                toast.error(`Row ${row.rowIndex}: Invalid Fee Type "${feeTypeName}".`);
                hasError = true;
                rowValid = false;
              } else {
                detail.feesType = feeType.id;
                console.log(`Row ${row.rowIndex}: FeeType ${feeTypeName} mapped to ID ${feeType.id}`);
              }

              if (!detail.installmentName || detail.totalFees <= 0 || detail.concessionPercentage < 0 || detail.concessionPercentage > 100) {
                toast.error(`Row ${row.rowIndex}: Invalid concession details.`);
                hasError = true;
                rowValid = false;
              }

              const expectedConcessionAmount = (detail.totalFees * detail.concessionPercentage) / 100;
              const expectedBalancePayable = detail.totalFees - expectedConcessionAmount;
              if (
                Math.abs(detail.concessionAmount - expectedConcessionAmount) > 0.01 ||
                Math.abs(detail.balancePayable - expectedBalancePayable) > 0.01
              ) {
                toast.error(`Row ${row.rowIndex}: Concession calculations incorrect.`);
                hasError = true;
                rowValid = false;
              }
            ...
            if (payload.concessionDetails.length === 0) {
              toast.error(`Row ${rows[0].rowIndex}: At least one concession detail is required.`);
              hasError = true;
              rowValid = false;
            }
            ...
            if (rowValid) {
              ...
              console.log(`AdmissionNumber ${admissionNumber}: Added to validatedData`, payload);
            } else {
              console.log(`AdmissionNumber ${admissionNumber}: Skipped due to validation errors`);
            }
          }

          if (hasError) {
            toast.warn('Some rows contain errors. Review the preview and correct the file.');
          } else {
    ...
    for (const [index, data] of validatedData.entries()) {
      try {
        ...
        data.concessionDetails.forEach((detail, idx) => {
          formDataToSend.append(`concessionDetails[${idx}][installmentName]`, detail.installmentName);
          formDataToSend.append(`concessionDetails[${idx}][feesType]`, detail.feesType);
          formDataToSend.append(`concessionDetails[${idx}][totalFees]`, detail.totalFees);
          formDataToSend.append(`concessionDetails[${idx}][concessionPercentage]`, detail.concessionPercentage);
          formDataToSend.append(`concessionDetails[${idx}][concessionAmount]`, detail.concessionAmount);
          formDataToSend.append(`concessionDetails[${idx}][balancePayable]`, detail.balancePayable);
        });
        ...
        if (!response.hasError) {
          toast.success(`Concession form for Admission Number ${data.AdmissionNumber} imported successfully (Row ${index + 1}).`);
        } else {
          toast.error(`Row ${index + 1}: ${response.message || 'Failed to import concession form.'}`);
        }
      } catch (err) {
        const errorMessage = err?.response?.data?.message || `Error importing concession form for row ${index + 1}.`;
