...
export const useConcessionForm = () => {
    ...
    useEffect(() => {
        if (student?.masterDefineClass?._id || student?.masterDefineClass) {
            ...
            const classSections = selectedClass?.sections.map(section => ({
                _id: section._id,
                name: section.name
            })) || [];
    ...
    const handleConcessionDetailChange = (index, e) => {
        ...
        if (name === 'concessionPercentage' || name === 'totalFees') {
            ...
            const percentage = parseFloat(updatedDetails[index].concessionPercentage) || 0;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
                    ...
                    if (value instanceof File) {
                        ...
                        value.forEach((item, index) => {
                            Object.entries(item).forEach(([field, val]) => {
                                formDataObj.append(`concessionDetails[${index}][${field}]`, val);
                            });
                        });
                    } else {
            ...
            const response = await putAPI(`/update-concession-form/${student._id}`, formDataObj, headers, true);
