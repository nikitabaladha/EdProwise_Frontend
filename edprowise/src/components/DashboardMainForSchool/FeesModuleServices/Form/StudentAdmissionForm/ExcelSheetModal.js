...
// const AdmissionExcelSheetModal = ({ show, onClose, schoolId, academicYear, onImportSuccess }) => {
...
//   //           payload.registrationNumber = row.registrationNumber?.toString().trim() || '';
...
//   //           const sectionObj = sections.find(s => s.name.toLowerCase() === sectionName?.toLowerCase());
//   //           if (!sectionObj) {
//   //             toast.error(`Row ${index + 1}: Invalid Section "${sectionName}" for class "${className}" and shift "${shiftName}".`);
//   //             hasError = true;
//   //             continue;
//   //           }
...
//   //           if (!feeType) {
//   //             toast.error(`Row ${index + 1}: Invalid Fee Type "${feeTypeName}" for class "${className}" and section "${sectionName}".`);
//   //             hasError = true;
//   //             continue;
//   //           }
...
//   //           //   toast.error(`Row ${index + 1}: Admission Fee must match the fee type amount (${feeType.amount}).`);
//   //           //   hasError = true;
//   //           //   continue;
//   //           // }
...
//   //             toast.error(`Row ${index + 1}: Concession Amount must be between 0 and Admission Fee (${admissionFees}).`);
//   //             hasError = true;
//   //             continue;
//   //           }
...
//   //             toast.error(`Row ${index + 1}: Final Amount must be Admission Fee (${admissionFees}) minus Concession (${concessionAmount}).`);
//   //             hasError = true;
//   //             continue;
//   //           }
...
//   //             { key: 'bloodGroup', value: payload.bloodGroup, label: 'Blood Group' },
...
//   //             { key: 'parentContactNumber', value: payload.parentContactNumber, label: 'Parent Contact Number' },
//   //             { key: 'motherTongue', value: payload.motherTongue, label: 'Mother Tongue' },
//   //             { key: 'aadharPassportNumber', value: payload.aadharPassportNumber, label: 'Aadhar/Passport Number' },
...
//   //             // { key: 'paymentMode', value: payload.paymentMode, label: 'Payment Mode' },
//   //           ];

//   //           // if (!isNursery) {
//   //           //   requiredFields.push(
//   //           //     { key: 'previousSchoolName', value: payload.previousSchoolName, label: 'Previous School Name' },
//   //           //     { key: 'addressOfPreviousSchool', value: payload.addressOfPreviousSchool, label: 'Address of Previous School' },
//   //           //     { key: 'previousSchoolBoard', value: payload.previousSchoolBoard, label: 'Previous School Board' }
//   //           //   );
//   //           // }
...
//   //               { key: 'fatherContactNo', value: payload.fatherContactNo, label: 'Father Contact Number' },
//   //               { key: 'fatherProfession', value: payload.fatherProfession, label: 'Father Profession' }
//   //             );
//   //           }
...
//   //               { key: 'motherContactNo', value: payload.motherContactNo, label: 'Mother Contact Number' },
//   //               { key: 'motherProfession', value: payload.motherProfession, label: 'Mother Profession' }
//   //             );
//   //           }

//   //           if (!payload.siblingInfoChecked) {
//   //             requiredFields.push(
//   //               { key: 'relationType', value: payload.relationType, label: 'Relation Type' },
//   //               { key: 'siblingName', value: payload.siblingName, label: 'Sibling Name' }
//   //             );
//   //           }

//   //           // if (payload.studentCategory !== 'General') {
//   //           //   requiredFields.push(
//   //           //     { key: 'castCertificate', value: payload.castCertificate, label: 'Caste Certificate' }
//   //           //   );
//   //           // }
...
//   //             toast.error(`Row ${index + 1}: Invalid Nationality "${payload.nationality}". Must be one of: India, International, SAARC Countries.`);
//   //             hasError = true;
//   //             continue;
//   //           }

//   //           if (!['Male', 'Female'].includes(payload.gender)) {
//   //             toast.error(`Row ${index + 1}: Invalid Gender "${payload.gender}". Must be one of: Male, Female.`);
//   //             hasError = true;
//   //             continue;
//   //           }

//   //           if (!['AB-', 'AB+', 'O-', 'O+', 'B-', 'B+', 'A-', 'A+'].includes(payload.bloodGroup)) {
//   //             toast.error(`Row ${index + 1}: Invalid Blood Group "${payload.bloodGroup}". Must be one of: AB-, AB+, O-, O+, B-, B+, A-, A+.`);
//   //             hasError = true;
//   //             continue;
//   //           }

//   //           if (!['General', 'OBC', 'ST', 'SC'].includes(payload.studentCategory)) {
//   //             toast.error(`Row ${index + 1}: Invalid Student Category "${payload.studentCategory}". Must be one of: General, OBC, ST, SC.`);
//   //             hasError = true;
//   //             continue;
//   //           }

//   //           if (!['Single Father', 'Single Mother', 'Parents'].includes(payload.parentalStatus)) {
//   //             toast.error(`Row ${index + 1}: Invalid Parental Status "${payload.parentalStatus}". Must be one of: Single Father, Single Mother, Parents.`);
//   //             hasError = true;
//   //             continue;
//   //           }

//   //           if (!payload.siblingInfoChecked && !['Brother', 'Sister'].includes(payload.relationType)) {
//   //             toast.error(`Row ${index + 1}: Invalid Relation Type "${payload.relationType}". Must be one of: Brother, Sister.`);
//   //             hasError = true;
//   //             continue;
//   //           }

//   //           if (!['Cash', 'Cheque', 'Online',""].includes(payload.paymentMode)) {
//   //             toast.error(`Row ${index + 1}: Invalid Payment Mode "${payload.paymentMode}". Must be one of: Cash, Cheque, Online.`);
//   //             hasError = true;
//   //             continue;
//   //           }
...
//   //             toast.error(`Row ${index + 1}: Age (${payload.age}) does not match calculated age (${age}) from Date of Birth.`);
//   //             hasError = true;
//   //             continue;
//   //           }
...
//           if (!payload.siblingInfoChecked && !['Brother', 'Sister'].includes(payload.relationType) && payload.relationType !== null) {
//             toast.error(`Row ${index + 1}: Invalid Relation Type "${payload.relationType}". Must be one of: Brother, Sister, null.`);
//             hasError = true;
//             continue;
//           }
...
//           if (data.hasOwnProperty(key) && key !== 'selectedFeeType' && key !== 'receiptNumber') {
...
//           toast.success(`Admission form for ${data.firstName} ${data.lastName} imported successfully (Row ${index + 1}).`);
//         } else {
//           toast.error(`Row ${index + 1}: ${response.message || 'Failed to import admission form.'}`);
//         }
//       } catch (err) {
//         const errorMessage = err?.response?.data?.message || `Error importing admission form for row ${index + 1}.`;
...
//     const guidelines = [
//       ['📌 Import Guidelines:'],
//       ['Required Fields: firstName, lastName, dateOfBirth, age, nationality, gender, bloodGroup, className, Shift, section, currentAddress, country, state, city, pincode, parentContactNumber, motherTongue, aadharPassportNumber, studentCategory, parentalStatus, selectedFeeType, admissionFees, finalAmount, name, paymentMode.'],
//       ['Optional Fields: registrationNumber, AdmissionNumber, middleName, concessionAmount, fatherQualification, motherQualification.'],
...
//       ['Formats: Dates must be YYYY-MM-DD. siblingInfoChecked must be true/false. paymentMode must be Cash/Cheque/Online. nationality must be India/International/SAARC Countries. gender must be Male/Female. bloodGroup must be AB-/AB+/O-/O+/B-/B+/A-/A+. studentCategory must be General/OBC/ST/SC. parentalStatus must be Single Father/Single Mother/Parents. relationType must be Brother/Sister.'],
//       ['admissionFees must match the selectedFeeType amount. finalAmount = admissionFees - concessionAmount.'],
//       ['AdmissionNumber: Optional. If provided, must match the format defined in PrefixSetting (e.g., ADM1001 for alphanumeric prefix "ADM"). If not provided, it will be auto-generated.'],
//       ['receiptNumber: Auto-generated by the system. Do not include in the import file.'],
...
//     const wsData = [
//       [
//         'registrationNumber', 'AdmissionNumber', 'firstName', 'middleName', 'lastName', 'dateOfBirth', 'age', 'nationality',
...
//         classes[0]?.sections[0]?.name || 'A', '123 Main St', 'India', 'Delhi', 'New Delhi',
...
//         'Admission Fee', '1000', '100', '900', 'Admin User', 'Cheque', '123456', 'State Bank'
//       ],
//     ];
...
//     writeFile(wb, 'admission_form.xlsx');
//   };
...
const AdmissionExcelSheetModal = ({ show, onClose, schoolId, academicYear, onImportSuccess }) => {
      ...
      try {
        ...
        reader.onload = async (e) => {
          ...
          for (let index = 0; index < jsonData.length; index++) {
            ...
            if (!payload.siblingInfoChecked && !['Brother', 'Sister', null].includes(payload.relationType)) {
              toast.error(`Row ${index + 1}: Invalid Relation Type "${payload.relationType}". Must be one of: Brother, Sister, null.`);
              hasError = true;
              continue;
            }
            ...
            if (!['Cash', 'Cheque', 'Online', 'null'].includes(payload.paymentMode)) {
              toast.error(`Row ${index + 1}: Invalid Payment Mode "${payload.paymentMode}". Must be one of: Cash, Cheque, Online, or null.`);
              hasError = true;
              continue;
            }
  ...
  const handleDownloadDemo = () => {
    const guidelines = [
      ['📌 Import Guidelines:'],
      ['Required Fields: firstName, lastName, dateOfBirth, age, nationality, gender, bloodGroup, className, Shift, section, currentAddress, country, state, city, pincode, parentContactNumber, motherTongue, aadharPassportNumber, studentCategory, parentalStatus, selectedFeeType, admissionFees, finalAmount, name.'],
      ['Optional Fields: registrationNumber, AdmissionNumber, middleName, concessionAmount, concessionType, fatherQualification, motherQualification, chequeNumber, bankName, paymentMode.'],
      ['Conditional Fields: chequeNumber and bankName are required if paymentMode is Cheque. fatherName, fatherContactNo, fatherProfession required if parentalStatus is not Single Mother. motherName, motherContactNo, motherProfession required if parentalStatus is not Single Father. relationType, siblingName required if siblingInfoChecked is false. concessionType is required and must be one of EWS, SC, ST, OBC, Staff Children, Other if concessionAmount is greater than 0; otherwise, it must be empty.'],
      ['Formats: Dates must be YYYY-MM-DD. siblingInfoChecked must be true/false. paymentMode must be Cash/Cheque/Online or empty. nationality must be India/International/SAARC Countries. gender must be Male/Female. bloodGroup must be AB-/AB+/O-/O+/B-/B+/A-/A+. studentCategory must be General/OBC/ST/SC. parentalStatus must be Single Father/Single Mother/Parents. relationType must be Brother/Sister or null. concessionType must be EWS/SC/ST/OBC/Staff Children/Other or empty.'],
      ['admissionFees must be a valid number. finalAmount = admissionFees - concessionAmount.'],
      ['AdmissionNumber: Optional. If provided, must match the format defined in PrefixSetting (e.g., ADM1001 for alphanumeric prefix "ADM"). If not provided, it will be auto-generated.'],
      ['receiptNumber: Auto-generated by the system. Do not include in the import file.'],
      ['File Fields: studentPhoto, previousSchoolResult, tcCertificate, proofOfResidence, aadharPassportFile, castCertificate, idCardFile are optional and can be uploaded later via the update form.'],
      ['selectedFeeType is required for validation and must match an existing feeType in the OneTimeFees collection.'],
      ['Do not change column headers; they must remain exactly as provided.'],
      ['If the payment mode is "Cash", "Online", or empty, leave the Cheque Number and Bank Name fields blank.'],
      ['Use the "Data" sheet to enter your data.'],
      ['The agreementChecked field is automatically set to true during import and should not be included in the Excel sheet.'],
      [`Available Classes: ${classes.map(c => c.className).join(', ')}.`],
      [`Available Shifts: ${shifts.map(s => s.masterDefineShiftName).join(', ')}.`],
