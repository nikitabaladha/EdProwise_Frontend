export const validateFullForm = (formData, isNursery) => {
    ...
    if (formData.parentalStatus === 'Single Father') {
        ...
        if (!phoneRegex.test(formData.fatherContactNo)) return 'Please enter a valid 10-digit father contact number';
        ...
        if (!phoneRegex.test(formData.motherContactNo)) return 'Please enter a valid 10-digit mother contact number';
        ...
        return 'Please select a valid parental status';
    }

    if (!formData.siblingInfoChecked) {
        if (!formData.relationType) return 'Please select relation type for sibling';
