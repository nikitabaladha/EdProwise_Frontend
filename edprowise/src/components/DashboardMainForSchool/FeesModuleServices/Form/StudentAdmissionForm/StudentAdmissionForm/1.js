...
//   const { student: initialStudent, feeTypeName, className, sectionName,classId,sectionId } = location.state || {};
...
//                 <td className="text-center">{student?.admissionFees.toFixed(2) || "0"}</td>
...
const FeesReceipt = () => {
  ...
  const [schoolData, setSchoolData] = useState({ school: null, logoSrc: '' });
  useEffect(() => {
  console.log("FeesReceipt received sectionId:", sectionId);
}, [sectionId]);
  ...
  const handleActionSelect = (action) => {
    ...
    console.log("handleActionSelect - sectionId before opening modal:", sectionId);
    setSelectedAction(action);
    setShowModal(true);
  };

  console.log("FeesReceipt - Passing sectionId to CancelReceiptModal:", sectionId);
