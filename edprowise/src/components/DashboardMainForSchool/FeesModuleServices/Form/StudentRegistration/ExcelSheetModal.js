        ...
        reader.onload = async (e) => {
          ...
          for (let index = 0; index < jsonData.length; index++) {
            ...
            if (registrationFee <= 0 || registrationFee !== feeType.amount) {
              toast.error(`Row ${index + 1}: Registration Fee must match the fee type amount (${feeType.amount}).`);
              hasError = true;
              continue;
            }
            ...
            if (concessionAmount < 0 || concessionAmount > registrationFee) {
              toast.error(`Row ${index + 1}: Concession Amount must be between 0 and Registration Fee (${registrationFee}).`);
              hasError = true;
              continue;
            }
            ...
            if (finalAmount !== (registrationFee - concessionAmount)) {
              toast.error(`Row ${index + 1}: Final Amount must be Registration Fee (${registrationFee}) minus Concession (${concessionAmount}).`);
              hasError = true;
              continue;
            }
            ...
            if (!['Teacher', 'Advertisement', 'Student', 'Online Search'].includes(payload.howReachUs)) {
              toast.error(`Row ${index + 1}: Invalid How Reach Us "${payload.howReachUs}". Must be one of: Teacher, Advertisement, Student, Online Search.`);
              hasError = true;
              continue;
            }
    ...
    for (const [index, data] of validatedData.entries()) {
      try {
        ...
        if (!response.hasError) {
          toast.success(`Registration form for ${data.firstName} ${data.lastName} imported successfully (Row ${index + 1}).`);
        } else {
          toast.error(`Row ${index + 1}: ${response.message || 'Failed to import registration form.'}`);
        }
      } catch (err) {
        const errorMessage = err?.response?.data?.message || `Error importing registration form for row ${index + 1}.`;
