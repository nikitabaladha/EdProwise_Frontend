export const validateBasicForm = (formData, toast, isNurseryClass) => {
    if (!formData) {
    ...
    const requiredFields = [
      ...
      { field: 'fatherContactNo', message: "Father's contact number is required" }, 
    ...
    if (formData.aadharPassportNumber && !(aadhaarPattern.test(formData.aadharPassportNumber) || passportPattern.test(formData.aadharPassportNumber))) {
        toast.error('Please enter a valid Aadhaar number (12 digits) or Passport number (1 letter followed by 7 digits)');
        return false;
    }

    if (formData.masterDefineClass && !isNurseryClass(formData.masterDefineClass)) {
