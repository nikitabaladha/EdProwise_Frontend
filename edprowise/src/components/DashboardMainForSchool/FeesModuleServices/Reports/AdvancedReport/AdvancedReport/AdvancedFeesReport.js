...
// import { exportToExcel, exportToPDF } from './AdvancedFeesReportExport';
...
//         const queryYearStart = parseInt(year.value.split('-')[0]);
//         const recordYearStart = parseInt(row.academicYear.split('-')[0]);
//         return recordYearStart > queryYearStart;
//       });
...
const AdvancedFeesReport = () => {
  ...
  const formatInstallmentName = (installment) => {
    if (!installment) return '-';
    const match = installment.match(/^(.*?)(20\d{2})$/);
    if (match) {
    ...
    try {
      ...
      const unifiedData = responses.flatMap((res, index) => {
        ...
        const feeRecords = res.data.data.map(item => ({
          ...item,
          totalDue: Number(item.totalDue || 0).toFixed(0),
          totalPaid: Number(item.totalPaid || 0).toFixed(0),
          totalConcession: Number(item.totalConcession || 0).toFixed(0),
          feeTypes: Object.fromEntries(
            Object.entries(item.feeTypes || {}).map(([type, value]) => [
              type,
              { ...value, totalPaid: Number(value.totalPaid || 0).toFixed(0) }
            ])
          ),
          isRefund: false,
        }));
            ...
            return {
              ...
              feeTypes: Object.fromEntries(
                feeTypeRefunds.map(feeType => [
                  feeType.feeType,
                  {
                    totalPaid: Number(feeType.refundAmountandcancelledAmount || 0).toFixed(0),
                  }
                ])
              ),
              totalDue: Number(totalRefundAmount).toFixed(0),
              totalPaid: Number(totalRefundAmount - totalConcession).toFixed(0),
              totalConcession: Number(totalConcession).toFixed(0),
              isRefund: true,
            };
          })
        );
  ...
  const totals = filteredData.reduce(
    (acc, row) => {
      feeTypes.forEach((type) => {
        if (row.feeTypes[type]) {
          const value = Number(row.feeTypes[type].totalPaid || 0);
          acc[type] = (Number(acc[type]) || 0) + value;
        }
      });
      acc.totalDue = (Number(acc.totalDue) || 0) + Number(row.totalDue || 0);
      acc.totalPaid = (Number(acc.totalPaid) || 0) + Number(row.totalPaid || 0);
  ...
  Object.keys(totals).forEach((key) => {
    totals[key] = Number(totals[key] || 0).toFixed(0);
  });
  ...
  const getFieldValue = (record, field) => {
    const fieldId = field.id;
    if (feeTypes.includes(fieldId)) {
      const value = record.feeTypes[fieldId]?.totalPaid || '0';
      return Number(value).toFixed(0);
    } else if (fieldId === 'totalDue' || fieldId === 'totalPaid' || fieldId === 'totalConcession' ) {
      return Number(record[fieldId] || 0).toFixed(0);
    } else if (fieldId === 'academicYear') {
      return formatAcademicYear(record[fieldId]) || '-';
    } else if (fieldId === 'installmentName') {
      return formatInstallmentName(record[fieldId]) || '-';
    }
    return record[fieldId] || '-';
  };
