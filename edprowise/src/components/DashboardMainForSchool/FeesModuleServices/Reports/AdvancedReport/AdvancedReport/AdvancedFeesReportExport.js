...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Advanced Fees Report');
  ...
  saveAs(
    ...
    `Advanced_Fees_Report_${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Advanced Fees Report - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `Advanced_Fees_Report_Report_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
