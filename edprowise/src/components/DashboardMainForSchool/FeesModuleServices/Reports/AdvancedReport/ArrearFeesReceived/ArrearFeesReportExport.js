...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Arrear Fees Received Report');
  ...
  saveAs(
    ...
    `Arrear_Fees_Received_Report_${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Arrear Fees Received Report - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `Arrear_Fees_Received_Report_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
