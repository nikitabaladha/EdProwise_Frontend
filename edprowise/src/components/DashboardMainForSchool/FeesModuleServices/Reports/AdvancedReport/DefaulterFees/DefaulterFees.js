...
const fetchFeeData = async (years) => {
  ...
  const handleSelectChange = (selectedOptions, { name }) => {
    ...
    if (name === 'academicYear') {
      ...
      setCurrentPage(1);
    } else if (name === 'tcStatus') { // Added for TCStatus
      setSelectedTCStatus(selectedOptions); // Single select, not multi
      setCurrentPage(1);
    }
  };
  ...
  const resetFilters = () => {
    ...
    setSelectedTCStatus(null); // Reset TCStatus
    setStartDate('');
    setEndDate('');
    setSearchTerm('');
    setCurrentPage(1);
  ...
  const processedData = feeData.flatMap((student) =>
    student.installments.map((installment) => ({
      ...
      tcStatus: student.tcStatus || 'Active', // Added TCStatus
    }))
  );
  ...
  const filteredData = processedData.filter((row) => {
    ...
    const rowDueDateParts = row.dueDate ? row.dueDate.split('/') : [];
    const rowDueDate = rowDueDateParts.length === 3 ? new Date(`${rowDueDateParts[2]}-${rowDueDateParts[1]}-${rowDueDateParts[0]}`) : null;
    const filterStart = startDate ? new Date(startDate) : null;
    const filterEnd = endDate ? new Date(endDate) : null;
    const matchesDate = (!filterStart || (rowDueDate && rowDueDate >= filterStart)) && (!filterEnd || (rowDueDate && rowDueDate <= filterEnd));
                    ...
                    <table className="table text-dark border border-secondary mb-1">
                      <thead>
                        <tr className="payroll-table-header">
                          ...
                          <th className="text-center align-middle border border-secondary text-nowrap p-2">Due Date</th>
                          <th className="text-center align-middle border border-secondary text-nowrap p-2"> Days O/S</th>
...
//         getAPI(`/Defaulter-Fees?schoolId=${schoolId}&academicYear=${year}`)
//       );
//       const lateAdmissionPromises = years.map((year) =>
//         getAPI(`/Loss-of-fee-due-to-late-Admission?schoolId=${schoolId}&academicYear=${year}`)
//       );


//       const [defaulterResponses, lateAdmissionResponses] = await Promise.all([
//         Promise.all(defaulterPromises),
//         Promise.all(lateAdmissionPromises),
//       ]);


//       const unifiedDefaulterData = defaulterResponses.flatMap((res, index) => {
//         if (!res?.data?.data) {
//           console.warn(`No defaulter data found for year ${years[index]}`);
//           return [];
//         }
...
//       const lateAdmissionNumbers = new Set(lateAdmissionData.map((record) => record.admissionNumber));


//       const filteredDefaulterData = unifiedDefaulterData.filter(
//         (record) => !lateAdmissionNumbers.has(record.admissionNumber)
//       );


//       console.log('Filtered Defaulter Data:', filteredDefaulterData);
...
//       if (rowsPerPage === 'all' && filteredDefaulterData.length > 0) {
//         const studentDataArray = Object.keys(
//           filteredDefaulterData.reduce((acc, row) => {
//             acc[row.admissionNumber] = {};
//             return acc;
//           }, {})
//         ).length;
