...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Loss of Fee Due to Late Admission');
  ...
  saveAs(
    ...
    `Loss_of_Fee_Due_to_Late_Admission_${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Loss of Fee Due to Late Admission - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `Loss_of_Fee_Due_to_Late_Admission_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
