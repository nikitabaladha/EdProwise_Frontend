...
// import { exportToExcel, exportToPDF } from './LateAdmssionFeeReport';
// import { fetchSchoolData } from '../../../PdfUtlisReport';

// const LossOfFeeDueToLateAdmssion = () => {
...
//       displayDate: installment.paymentDate,
...
//       (row.displayDate === '-' ? true : selectedPaymentModes.some((mode) => row.paymentMode === mode.value));
...
//                   <h2 className="payroll-title text-center mb-0 flex-grow-1">Loss of Fee Due to Late Admssion</h2>
//                 </div>
//               </div>
...
// export default LossOfFeeDueToLateAdmssion;
...
const LossOfFeeDueToLateAdmission = () => {
  const [showFilterPanel, setShowFilterPanel] = useState(false);
  const [showExportDropdown, setShowExportDropdown] = useState(false);
  const [activeTab, setActiveTab] = useState('Date');
  const [searchTerm, setSearchTerm] = useState('');
  ...
  const processedData = feeData.flatMap((student) =>
    student.installments.map((installment) => ({
      ...
      sectionName: student.sectionName,
      academicYear: student.academicYear,
      displayDate: student.admissionPaymentDate, // Use admissionPaymentDate from AdmissionForm
      feesDue: installment.feesDue || 0,
      feesPaid: installment.feesPaid || 0,
      concession: installment.concession || 0,
      balance: installment.balance || 0,
  ...
  const groupedByStudent = filteredData.reduce((acc, row) => {
    ...
    if (!acc[admissionNumber]) {
      acc[admissionNumber] = {
        ...
        sectionName: row.sectionName,
        academicYear: row.academicYear,
        displayDate: row.displayDate, // Use admissionPaymentDate for grouping
        rows: [],
      };
    }
    acc[admissionNumber].rows.push(row);
        ...
        <div className="col-md-12">
          <div className="card m-2">
            <div className="card-body p-2">
              ...
              <div className="container">
                <div className="card-header d-flex justify-content-between align-items-center gap-1">
                  <h2 className="payroll-title text-center mb-0 flex-grow-1">Loss of Fee Due to Late Admission</h2>
                </div>
              </div>
                    ...
                    <table className="table text-dark border border-secondary mb-1">
                      <thead>
                        <tr className="payroll-table-header">
                          ...
                          <th className="text-center align-middle border border-secondary text-nowrap p-2">Concession</th>
                          <th className="text-center align-middle border border-secondary text-nowrap p-2">Balance</th>
                          <th className="text-center align-middle border border-secondary text-nowrap p-2">Admission Date</th>
                        </tr>
                      </thead>
                      <tbody>
                        {paginatedData().map((student, studentIndex) => (
