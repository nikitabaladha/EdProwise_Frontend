...
export const exportToExcel = async (
  ...
  try {
    ...
    const worksheet = workbook.addWorksheet('Opening And Closing Arrear Report');
    ...
    filteredData.forEach((record, index) => {
      const row = tableFields.map((field) => {
        ...
        return typeof value === 'string' && !isNaN(value) && value !== '' ? Number(value) : value;
      });
    ...
    const totalsRow = tableFields.map((field) => {
      if (field.id === 'academicYear') return '';
      if (field.id === 'studentName') return 'Total';
      const fieldMapping = {
        openingArrear: 'totalOpeningArrear',
        feesReceived: 'totalFeesReceived',
        defaulterFeesTransferred: 'totalDefaulterFeesTransferred',
        closingBalance: 'totalClosingBalance',
      };
      const totalField = fieldMapping[field.id];
      return totalField ? Number(totals[totalField] || 0).toFixed(2) : '0.00';
    });
    ...
    const fileName = `Opening_And_Closing_Arrear_Report_${formatAcademicYear(selectedYears || 'Unknown')}.xlsx`;
...
export const exportToPDF = async (
  ...
  try {
    ...
    if (filteredData.length > 0) {
      pageData[pageData.length - 1] = [
        ...pageData[pageData.length - 1],
        {
          ...
          academicYear: '',
          studentName: 'Total',
          openingArrear: Number(totals.totalOpeningArrear).toFixed(2),
          feesReceived: Number(totals.totalFeesReceived).toFixed(2),
          defaulterFeesTransferred: Number(totals.totalDefaulterFeesTransferred).toFixed(2),
          closingBalance: Number(totals.totalClosingBalance).toFixed(2),
        },
      ];
    } else {
      pageData.push([{
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Opening And Closing Arrear Report - ${formatAcademicYear(selectedYears || 'Unknown')}</div>
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            ${currentPageData.length > 0
              ? currentPageData
                    ...
                    if (record.isTotalsRow) {
                      return `
                        <tr>
                          ...
                          <td><strong>${Number(totals.totalOpeningArrear).toFixed(2)}</strong></td>
                          <td><strong>${Number(totals.totalFeesReceived).toFixed(2)}</strong></td>
                          <td><strong>${Number(totals.totalDefaulterFeesTransferred).toFixed(2)}</strong></td>
                          <td><strong>${Number(totals.totalClosingBalance).toFixed(2)}</strong></td>
                        </tr>
                      `;
                    }
                    return `
                  ...
                  .join('')
              : `<tr><td colspan="${tableFields.length}">No data matches the selected filters for ${formatAcademicYear(selectedYears || 'Unknown')}.</td></tr>`
            }
          </tbody>
        </table>
      `;
      ...
      if (destHeight > 0) {
        ...
        console.warn(`Page ${page + 1} - Table height is 0, rendering placeholder text.`);
    ...
    const fileName = `Opening_And_Closing_Arrear_Report_${formatAcademicYear(selectedYears || 'Unknown')}.pdf`;
    pdf.save(fileName);
  } catch (error) {
