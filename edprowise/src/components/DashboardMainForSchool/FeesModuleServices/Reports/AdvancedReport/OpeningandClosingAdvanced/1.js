...
const AdvancedFeesReport = () => {
  ...
  const tableFields = [
    ...
    ...feeTypes.flatMap((type) => [
      { id: `${type}_openingAdvance`, label: `${type} Opening Advance` },
      { id: `${type}_received`, label: `${type} Received` },
      { id: `${type}_adjusted`, label: `${type} Adjusted` },
      { id: `${type}_closingBalance`, label: `${type} Closing Balance` },
    ]),
  ...
  const getFieldValue = (record, field) => {
    ...
    if (fieldId === 'totalReceived') {
      return record[fieldId] || 0;
    }
    const [feeType, prop] = fieldId.split('_');
    if (feeType && prop && record.feeTypes[feeType]) {
      return record.feeTypes[feeType][prop] || 0;
    }
                            ...
                            onClick={async () => {
                              ...
                              try {
                                await exportToExcel(
                                  filteredData,
                                  tableFields,
                                  tableFields.reduce((acc, field) => ({ ...acc, [field.id]: field.label }), {}),
                                  getFieldValue,
                                  totals,
                                  formatAcademicYear,
                                  selectedYears.length > 0
