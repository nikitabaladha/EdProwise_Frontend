...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Opening And Closing Advanced Report');
  ...
  saveAs(
    ...
    `Opening_And_Closing_Advanced_Report_${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Opening And Closing Advanced Report - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `Opening_And_Closing_Advanced_Report_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
