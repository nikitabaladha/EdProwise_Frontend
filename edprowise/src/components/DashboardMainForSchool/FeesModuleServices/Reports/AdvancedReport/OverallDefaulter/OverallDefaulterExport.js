...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Over All Outstanding');
  ...
  saveAs(
    ...
    `Over_All_Outstanding${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Over All Outstanding - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `Over_All_Outstanding${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
