...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('StudentWise Fees Due Report');
  ...
  saveAs(
    ...
    `StudentWise_Fees_Due_Report${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">StudentWise_Fees_Due_Report - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `StudentWise_Fees_Due_Report${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
