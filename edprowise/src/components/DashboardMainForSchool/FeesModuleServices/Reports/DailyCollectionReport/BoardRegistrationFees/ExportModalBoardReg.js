...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Board Registration Fees');
  ...
  saveAs(
    ...
    `Board_Registration_Fees_${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Board Registration Fees Report - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `Board_Registration_Fees_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
