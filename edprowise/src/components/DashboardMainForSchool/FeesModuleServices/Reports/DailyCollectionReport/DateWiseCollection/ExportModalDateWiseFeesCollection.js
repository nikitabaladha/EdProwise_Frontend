...
//     // Group data as done in the React component
//     const groupedData = filteredData.reduce((acc, record, index) => {
//       const key = `${record.paymentDate}_${record.academicYear}_${record.paymentMode}`;
//       if (!acc[key]) {
//         acc[key] = { records: [], count: 0 };
//       }
//       acc[key].records.push({ record, index });
//       acc[key].count += 1;
//       return acc;
//     }, {});
...
//     const fileName = `Datewise_Collection_INC_Concession_${formatAcademicYear(selectedYears)}.pdf`;
//     pdf.save(fileName);
//   } catch (error) {
...
export const exportToExcel = async (
  ...
  try {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet(`Datewise Collection INC Concession (${viewMode})`);
    ...
    saveAs(
      new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }),
      `Datewise_Collection_INC_Concession_${viewMode}_${formatAcademicYear(selectedYears)}.xlsx`
    );
  } catch (error) {
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Datewise Collection INC Concession (${viewMode.toUpperCase()}) - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
              ${tableFields.map((field) => `<th>${headerMapping[field.id] || field.label}</th>`).join('')}
    ...
    document.body.removeChild(hiddenContainer);
    const fileName = `Datewise_Collection_INC_Concession_${viewMode}_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
    console.error('PDF generation failed:', error);
    toast.error('Failed to generate PDF. Please check the console for details.');
