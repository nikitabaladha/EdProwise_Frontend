...
export const exportToExcel = async (
  ...
  try {
    ...
    const worksheet = workbook.addWorksheet(`Datewise Collection EXC Concession (${viewMode})`);
    ...
    saveAs(
      ...
      `Datewise_Collection_EXC_Concession_${viewMode}_${formatAcademicYear(selectedYears)}.xlsx`
    );
  } catch (error) {
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Datewise Collection EXC Concession (${viewMode.toUpperCase()}) - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `Datewise_Collection_EXC_Concession_${viewMode}_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
