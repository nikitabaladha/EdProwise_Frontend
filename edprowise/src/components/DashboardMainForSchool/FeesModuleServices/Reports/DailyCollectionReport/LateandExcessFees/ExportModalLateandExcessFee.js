...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Late and Excess Fees');
  ...
  const totalsRow = tableFields.map(field => {
    if (field.id === 'paidFine') return Number(totals.totalPaidFine);
    else if (field.id === 'excessFees') return Number(totals.totalExcessFees);
    else if (field.id === 'total') return Number(totals.total);
    return '';
  });
  ...
  saveAs(
    ...
    `Late_and_Excess_Fees_${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Late and Excess Fees Report - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
                    ...
                    if (record.isTotalsRow) {
                      return `
                        <tr>
                          ...
                          <td><strong>${totals.totalPaidFine}</strong></td>
                          <td><strong>${totals.totalExcessFees}</strong></td>
    ...
    const fileName = `Late_and_Excess_Fees_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
