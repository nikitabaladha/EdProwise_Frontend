...
export const exportToExcel = async (
  ...
  try {
    ...
    const buffer = await workbook.xlsx.writeBuffer();
    console.log('Saving Excel file...');
    const fileName = `Monthwise_Collection_INC_Concession_${formatAcademicYear(selectedYears || '')}_${viewMode}.xlsx`;
    saveAs(
      new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }),
      fileName
    );
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Monthwise Collection INC Concession - ${formatAcademicYear(selectedYears)} (${viewMode.charAt(0).toUpperCase() + viewMode.slice(1)})</div>
        <table>
          <thead>
            <tr>
              ${tableFields.map((field) => `<th>${headerMapping[field.id] || field.label}</th>`).join('')}
    ...
    document.body.removeChild(hiddenContainer);
    const fileName = `Monthwise_Collection_INC_Concession_${formatAcademicYear(selectedYears || '')}_${viewMode}.pdf`;
    console.log('Saving PDF:', fileName);
    pdf.save(fileName);
    console.log('PDF export completed.');
  } catch (error) {
