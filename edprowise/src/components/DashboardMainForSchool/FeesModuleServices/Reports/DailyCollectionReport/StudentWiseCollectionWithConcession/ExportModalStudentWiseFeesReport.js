...
export const exportToExcel = async (
  ...
  try {
    ...
    const worksheet = workbook.addWorksheet('Studentwise Collection Exc Concession Report');
    ...
    saveAs(blob, `Studentwise_Collection_Exc_Concession_Report_${formatAcademicYear(selectedYears)}.xlsx`);
  } catch (error) {
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Studentwise Collection Exc Concession Report - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `Studentwise_Collection_Exc_Concession_Report_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
