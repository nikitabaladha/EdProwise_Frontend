...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Student-Wise Fees Report With Concession');
  ...
  saveAs(
    ...
    `Student_Wise_Fees_Report_With_Concession_${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Student-Wise Fees Report With Concession - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `Student_Wise_Fees_Report_With_Concession${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
