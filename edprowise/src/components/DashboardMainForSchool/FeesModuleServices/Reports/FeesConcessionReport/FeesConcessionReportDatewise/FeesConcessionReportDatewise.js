...
useEffect(() => {
  ...
  const fetchFeeData = async () => {
    setIsLoading(true);
    try {
      let apiUrl = `/get-all-studentwise-concession-report?schoolId=${schoolId}`;
      ...
      const unifiedData = data.flatMap((record) => {
        const totalPaidFee = Object.values(record.feeTypes || {}).reduce(
          (sum, amount) => sum + (Number(amount) || 0),
          0
        );
        ...
        if (record.cancelledDate) {
          const negativeFeesBreakdown = {};
          Object.keys(record.feeTypes || {}).forEach((key) => {
            negativeFeesBreakdown[key] = -(Number(record.feeTypes[key]) || 0);
          });
          ...
          const cancelledRecord = {
            ...
            paymentDate: record.cancelledDate,
            feesBreakdown: negativeFeesBreakdown,
            totalPaidFee: -Math.abs(totalPaidFee || 0),
            status: 'Cancelled',
          };

          return [normalRecord, cancelledRecord];
        }
      ...
      unifiedData.sort((a, b) => {
        const dateA = new Date(a.paymentDate?.split('-').reverse().join('-') || 0);
        const dateB = new Date(b.paymentDate?.split('-').reverse().join('-') || 0);
        ...
        const isACancellation =
          a.cancelledDate || Object.values(a.feesBreakdown).some((amt) => Number(amt) < 0);
        const isBCancellation =
          b.cancelledDate || Object.values(b.feesBreakdown).some((amt) => Number(amt) < 0);
        return isACancellation === isBCancellation ? 0 : isACancellation ? 1 : -1;
      });
