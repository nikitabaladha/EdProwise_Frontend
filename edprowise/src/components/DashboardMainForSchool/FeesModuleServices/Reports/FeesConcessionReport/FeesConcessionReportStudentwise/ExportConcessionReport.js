...
export const exportToExcel = async (
  ...
  try {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet(`Studentwise  Concession (${viewMode.toUpperCase()})`);
    ...
    saveAs(
      ...
      `Studentwise_Concession_${viewMode}_${formatAcademicYear(selectedYears)}.xlsx`
    );
  } catch (error) {
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">StudentWise  Concession (${viewMode.toUpperCase()}) - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    document.body.removeChild(hiddenContainer);
    const fileName = `Studentwise_Concession_${viewMode}_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
