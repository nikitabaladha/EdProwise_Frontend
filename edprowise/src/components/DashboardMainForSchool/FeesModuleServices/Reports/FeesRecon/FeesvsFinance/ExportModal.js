...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Fees vs Finance');
  ...
  const totalsRow = tableFields.map(field => {
    if (field.id === 'feesModuleAmount') return Number(totals.feesModuleAmount).toFixed(2);
    else if (field.id === 'financeModuleAmount') return Number(totals.financeModuleAmount).toFixed(2);
    else if (field.id === 'difference') return Number(totals.difference).toFixed(2);
    return field.id === 'feesTypeName' ? 'Total' : '';
  });
  ...
  saveAs(
    ...
    `Fees_vs_Finance_${formatAcademicYear(selectedAcademicYear)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Fees vs Finance Reconciliation Report - ${formatAcademicYear(selectedAcademicYear)}</div>
        <table>
          <thead>
            <tr>
                    ...
                    if (record.isTotalsRow) {
                      return `
                        <tr>
                          ...
                          <td><strong>${Number(totals.feesModuleAmount).toFixed(2)}</strong></td>
                          <td><strong>${Number(totals.financeModuleAmount).toFixed(2)}</strong></td>
                          <td><strong>${Number(totals.difference).toFixed(2)}</strong></td>
                        </tr>
                      `;
                    }
                    return `
                      <tr>
                        <td>${record.feesTypeName}</td>
                        <td>${Number(record.feesModuleAmount).toFixed(2)}</td>
                        <td>${Number(record.financeModuleAmount).toFixed(2)}</td>
                        <td>${Number(record.difference).toFixed(2)}</td>
                      </tr>
                    `;
                  })
                  .join('')
    ...
    pdf.save(`Fees_vs_Finance_${formatAcademicYear(selectedAcademicYear)}.pdf`);
  } catch (error) {
