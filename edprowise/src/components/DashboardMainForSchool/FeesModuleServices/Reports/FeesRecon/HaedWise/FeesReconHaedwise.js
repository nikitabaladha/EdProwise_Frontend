...
//                         </tr>
//                         <tr>
//                           <td colSpan={5} className="text-right border border-dark p-2">
...
//                         </tr>
//                         <tr className="payroll-table-footer">
//                           <td colSpan={5} className="text-right border border-dark p-2"><strong>Net Fees Due</strong></td>
...
//                         </tr>
//                         <tr className="payroll-table-footer">
//                           <td colSpan={5} className="text-right border border-dark p-2"><strong>Total Fees Received Inc Concession</strong></td>
//                           <td className="text-center border border-dark p-2"><strong>{finalFees}</strong></td>
//                         </tr>
//                         <tr className="payroll-table-footer">
//                           <td colSpan={5} className="text-right border border-dark p-2"></td>
//                           <td className="text-center border border-dark p-2"><strong></strong></td>
//                         </tr>
//                           <tr className="payroll-table-footer">
//                           <td colSpan={5} className="text-right border border-dark p-2"><strong>Total Fees Received Incl Concession (As per Daily Collection)</strong></td>
//                           <td className="text-center border border-dark p-2"><strong></strong></td>
//                         </tr>
//                           <tr className="payroll-table-footer">
//                           <td colSpan={5} className="text-right border border-dark p-2"><strong>Difference</strong></td>
//                           <td className="text-center border border-dark p-2"><strong></strong></td>
//                         </tr>
//                       </tbody>
//                     </table>
...
const ReconFeesHeadwise = () => {
  ...
  const [boardExaminationFees, setBoardExaminationFees] = useState(0);
  const [arrearFeesReceived, setArrearFeesReceived] = useState(0);
  const [dailyCollectionTotal, setDailyCollectionTotal] = useState(0); // New state for daily collection total
  const dropdownRef = useRef(null);
  const tabs = ['Academic Year', 'Class & Section', 'Installment'];
  ...
  const fetchDailyCollectionTotal = async () => {
    try {
      const userDetails = JSON.parse(localStorage.getItem('userDetails'));
      ...
      const yearsToFetch = [selectedAcademicYear];
      const promises = yearsToFetch.map((year) =>
        getAPI(`/get-all-data-datewise-fees?schoolId=${schoolId}&academicYear=${year}`)
      );
      
      const responses = await Promise.all(promises);
      const unifiedData = responses.flatMap((res, index) => {
        if (!res?.data?.data) {
          console.warn(`No data found for year ${yearsToFetch[index]}`);
          return [];
        }
      ...
      // Calculate total from all records (net view)
      const total = unifiedData.reduce((sum, record) => {
        return sum + (Number(record.totalPaidFee) || 0);
      }, 0);

      setDailyCollectionTotal(total);
    } catch (error) {
      console.error('Error fetching daily collection data:', error);
      setDailyCollectionTotal(0);
    }
  };
    ...
    const fetchData = async () => {
      ...
      try {
       ...
       setAdmissionFeesData(admissionFeesResponse.data?.combinedDetails || []);
        ...
        // Fetch daily collection total after all other data is loaded
        await fetchDailyCollectionTotal();
  ...
  // Also fetch daily collection when academic year changes
  useEffect(() => {
    if (schoolId && selectedAcademicYear) {
      fetchDailyCollectionTotal();
    }
  }, [selectedAcademicYear]);
...
useEffect(() => {
  ...
  filteredLateFees.forEach((item) => {
    const lateAmount =
      (parseFloat(item.paidFine) || 0) + (parseFloat(item.excessFees) || 0);


    totalLateAndExcess += lateAmount;

  
    if (item.reportStatus?.includes("Cancelled")) {
      totalLateAndExcess -= lateAmount;
    }
  });
  ...
  const netFeesDue = (Number(totals.yearlyDues) - Number(lossDueToLeft) - Number(lossDueToLateAdmission) - Number(defaulterFees)).toFixed(2);
  const finalFees = (Number(netFeesDue) + Number(lateAndExcessFees) +Number(admissionFees)+ Number(registrationFees) + Number(tcFees) + Number(boardRegistrationFees) + Number(boardExaminationFees) + Number(arrearFeesReceived)).toFixed(2);
  
  const difference = (Number(finalFees) - Number(dailyCollectionTotal)).toFixed(2);
                    ...
                    <table className="table text-dark border border-dark">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td className="text-center border border-dark p-2">{admissionFees}</td>
                        </tr>
                        <tr>
                          <td colSpan={5} className="text-right border border-dark p-2">
                            Add: Registration Fees
                        ...
                        <tr className="payroll-table-footer">
                          <td colSpan={5} className="text-right border border-dark p-2"><strong>Total Fees Received Incl Concession (As per Daily Collection)</strong></td>
                          <td className="text-center border border-dark p-2"><strong>{dailyCollectionTotal.toFixed(2)}</strong></td>
                        </tr>
                        <tr className="payroll-table-footer">
                          <td colSpan={5} className="text-right border border-dark p-2"><strong>Difference</strong></td>
                          <td className="text-center border border-dark p-2"><strong>{difference}</strong></td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
