...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Fees Recon Student Wise report');
  ...
  saveAs(
    ...
    `Fees_Recon_Student_Wise_Report_${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Fees Recon Student Wise - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `Fees_Recon_Student_Wise_Report_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
