...
// import { exportToExcel, exportToPDF } from './FeeReconStudentWiseExport';
...
//                   <h2 className="payroll-title text-center mb-0 flex-grow-1">Fees Recon Student Wise</h2>
//                 </div>
//               </div>
...
// export default FeeReconStudentWiseReport;
...
const SchoolFeesReport = () => {
  ...
  const fetchFeeData = async (years) => {
    ...
    try {
      const promises = years.map((year) =>
        getAPI(`/get-fees-recon-student-wise?schoolId=${schoolId}&academicYear=${year}`)
      );
      const responses = await Promise.all(promises);
      const unifiedData = responses.flatMap((res, index) => {
        if (!res?.data?.data) {
