...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Board Exam Fees');
  ...
  const totalsRow = tableFields.map(field => {
    if (field.id === 'boardExamFeesDue') return Number(totals.feesDue);
    else if (field.id === 'boardExamFeesPaid') return Number(totals.feesPaid);
    else if (field.id === 'boardExamFeesRefundAmount') return Number(totals.refund);
    else if (field.id === 'netFees') return Number(totals.netFees);
    else if (field.id === 'boardExamFeesConcession') return Number(totals.concession);
  ...
  saveAs(
    ...
    `Board_Exam_Fees_${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Board Exam Fees Report - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            ${currentPageData.length > 0
              ? currentPageData
                  ...
                  .join('')
              : `<tr><td colspan="${tableFields.length + 1}">No data matches the selected filters for ${formatAcademicYear(selectedYears)}.</td></tr>`
            }
          </tbody>
        </table>
      `;
    ...
    const fileName = `Board_Exam_Fees_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
