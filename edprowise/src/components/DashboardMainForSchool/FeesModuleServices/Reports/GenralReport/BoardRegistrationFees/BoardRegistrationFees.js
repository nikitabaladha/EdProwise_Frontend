...
const BoardRegistrationFees = () => {
  ...
  const [classSectionMap, setClassSectionMap] = useState({}); // New state for class-section mapping
  ...
  const fetchFeeData = async (years) => {
    ...
    try {
      ...
      unifiedData.sort((a, b) => {
        ...
        const dateB = b.student?.boardRegFeesFeesStatus.includes('Cancelled') || a.student?.boardRegFeesFeesStatus.includes('Cheque Return')
