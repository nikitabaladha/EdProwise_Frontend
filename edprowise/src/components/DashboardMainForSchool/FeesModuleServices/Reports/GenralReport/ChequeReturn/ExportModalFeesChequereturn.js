...
export const exportToExcel = async (
  ...
  const worksheet = workbook.addWorksheet('Fees Cheque Return');
  ...
  saveAs(
    ...
    `Fees_Cheque_Return_Report_${formatAcademicYear(selectedYears)}.xlsx`
  );
};
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Fees Cheque Return - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
                          ...
                          ${tableFields
                            ...
                            .map((field) =>
                              field.id === 'totalPaid'
                                ...
                                : `<td><strong>${(totals[field.id] ||'')}</strong></td>`
                            )
                            .join('')}
                        </tr>
                      `;
    ...
    const fileName = `Fees_Cheque_Return_Report_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
  } catch (error) {
