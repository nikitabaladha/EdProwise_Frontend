...
import { exportToExcel, exportToPDF } from './ExportModalFeesChequereturn';
import { fetchSchoolData } from '../../../PdfUtlisReport';

const FeesChequeReturn = () => {
  ...
  //          getAPI(`/get-all-Fees-cheque-return-report?schoolId=${schoolId}&academicYear=${year}${startDate ? `&startDate=${startDate}` : ''}${endDate ? `&endDate=${endDate}` : ''}`)
  //        );
  ...
  //                    <h2 className="payroll-title text-center mb-0 flex-grow-1">Fees Cheque Return Report</h2>
  //                  </div>
  //                </div>
  ...
  useEffect(() => {
    ...
    const fetchFeeData = async () => {
      ...
      try {
        ...
        let apiUrl = `/get-all-Fees-cheque-return-report?schoolId=${schoolId}`;
        ...
        const unifiedData = data.map((record) => {
          let feeTypes = {};
          if (record.feeType && Array.isArray(record.feeType)) {
            record.feeType.forEach((fee) => {
              if (fee.feeType) {
        ...
        const allFeeTypes = [
          ...new Set(
            unifiedData.flatMap((record) =>
              Object.keys(record.feeTypes).filter(Boolean)
            )
          ),
        ].sort();
...
export default FeesChequeReturn;