...
const FeesStructureReport = () => {
  ...
  const fetchFeeData = async () => {
    ...
    try {
      ...
      if (!response.hasError && response.data?.data) {
        ...
        const uniqueInstallments = [...new Set(data.map(item => item.installment).filter(inst => inst && inst !== '-'))].sort();
        setInstallmentOptions(uniqueInstallments.map(inst => ({ value: inst, label: inst })));

        if (rowsPerPage === 'all' && data.length > 0) {
  ...
  const paginatedRows = () => {
    ...
    return enhancedRows.slice(startIndex, endIndex);
  };
                      ...
                      <tbody>
                        {paginatedRows().map((item, index) => {
                          if (item.type === 'data') {
                            ...
                            return (
                              <tr key={`total_${item.className}_${item.sectionName}_${index}`} className="payroll-table-footer">
