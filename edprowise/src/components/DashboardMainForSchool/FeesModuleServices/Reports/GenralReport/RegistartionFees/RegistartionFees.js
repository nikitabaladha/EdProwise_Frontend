...
useEffect(() => {
  ...
  const fetchInitialData = async () => {
    setIsLoading(true);
    try {
      let apiUrl = `/get-all-data-Registration?schoolId=${schoolId}`;
  ...
  const paginatedData = () => {
    ...
    let currentCount = 0;
    const startIndex = rowsPerPage === 'all' ? 1: (currentPage - 1) * rowsPerPage;
...
value={
    rowsPerPage === 'all' 
      ? pageShowOptions.find(opt => opt.value === 'all')
      : pageShowOptions.find(opt => opt.value === rowsPerPage)
  }
                  ...
                  <div className="row mt-1 border border-light rounded px-md-3 py-1">
                    <div className="col-12 p-2">
                      ...
                      <div className="text-end mt-3">
                        ...
                        <button className="btn btn-primary">Close Filters</button>
                      </div>
                    </div>
                  </div>
                )}
