...
export const exportToExcel = async (
  ...
  try {
    ...
    const worksheet = workbook.addWorksheet(`School Fees INC Concession (${viewMode.toUpperCase()})`);
    ...
    saveAs(
      ...
      `School_Fees_INC_Concession_${viewMode}_${formatAcademicYear(selectedYears)}.xlsx`
    );
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">School Fees INC Concession (${viewMode.toUpperCase()}) - ${formatAcademicYear(selectedYears)}</div>
        <table>
          <thead>
            <tr>
    ...
    const fileName = `School_Fees_INC_Concession_${viewMode}_${formatAcademicYear(selectedYears)}.pdf`;
    pdf.save(fileName);
