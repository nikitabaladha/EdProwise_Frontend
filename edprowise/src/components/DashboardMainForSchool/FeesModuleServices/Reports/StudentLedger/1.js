...
const StudentFeeLedger = () => {
  ...
  const processedStudentData = students.map((student) => {
    ...
    // 3. Process Cancelled Payments (only once)
    ...
    cancelledPayments.forEach((payment) => {
      ...
      // Cancelled Due (reverse the original due)
      runningBalance += paidAmount;
      addTransaction({
        academicYear: payment.academicYear,
        date: formattedDate,
      ...
      // Cancelled Received (reverse the original receipt)
      runningBalance -= paidAmount;
      addTransaction({
        academicYear: payment.academicYear,
        date: formattedDate,
    ...
    // 5. Process Fee Installments from feeData (exclude cancelled payments)
    feeData.forEach((year) => {
      // Process due fees
      const feeAggregates = {};
      year.feeInstallments?.forEach((installment) => {
