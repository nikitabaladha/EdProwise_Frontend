...
const StudentFeeLedger = () => {
  ...
  const [selectedAcademicYear, setSelectedAcademicYear] = useState(
    localStorage.getItem('selectedAcademicYear') || '2026-2027'
  );
    ...
    const fetchFeeData = async () => {
      try {
        ...
        const response = await getAPI(
          `/get-all-data?schoolId=${schoolId}&admissionNumber=${admissionNumber}`
        );
  ...
  const processedStudentData = feeData.map((year) => {
    const student = year.student || {};
    ...
    const classInfo = classes.find((c) => c._id === year.classId) || { className: 'N/A' };
    const sectionInfo = classInfo?.sections?.find((s) => s._id === year.sectionId) || { name: 'N/A' };
    ...
    Object.keys(installmentGroups).forEach((academicYear) => {
      Object.keys(installmentGroups[academicYear])
        .sort((a, b) => {
          const dueDateA = year.feeInstallments.find((fi) => fi.installmentName === a)?.dueDate;
          const dueDateB = year.feeInstallments.find((fi) => fi.installmentName === b)?.dueDate;
    ...
    return {
      ...
      regFeesDate: student.regFeesDate
        ? new Date(student.regFeesDate).toLocaleDateString('en-GB', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
          }).replace(/\//g, '-')
      ...
      admFeesDate: student.admFeesDate
        ? new Date(student.admFeesDate).toLocaleDateString('en-GB', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
          }).replace(/\//g, '-')
      ...
      boardExamFeesDate: student.boardExamFeesDate
        ? new Date(student.boardExamFeesDate).toLocaleDateString('en-GB', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
          }).replace(/\//g, '-')
      ...
      boardRegFeesDate: student.boardRegFeesDate
        ? new Date(student.boardRegFeesDate).toLocaleDateString('en-GB', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
          }).replace(/\//g, '-')
  ...
  const academicYearOptions = feeData.map((year) => ({
                ...
                <div className="row p-1 border border-dark" style={{ background: '#bfbfbf' }}>
                  ...
                  <div className="col-md-3 col-12">
                    <CreatableSelect
                      ...
                      options={[{ value: 'All', label: 'All Years' }, ...academicYearOptions]}
                      value={
                        academicYearOptions.find((option) => option.value === selectedAcademicYear) ||
                        { value: 'All', label: 'All Years' }
                      }
                      onChange={(option) => setSelectedAcademicYear(option ? option.value : 'All')}
                      className="border border-dark"
                    />
                  </div>
              ...
              {shouldDisplayTable ? (
                filteredStudents.length > 0 ? (
                                ...
                                <tr key={tIndex} className="payroll-table-row">
                                  {tableFields.length > 0 ? (
                                    tableFields.map((field) => (
                                      ...
                                      >
                                        {transaction[field.id] || student[field.id] || 'N/A'}
                                      </td>
                                    ))
                                  ) : (
                  ...
                  <div className="text-center mt-3">
                    <p>No student data matches the selected search or academic year.</p>
                  </div>
                )
              ) : (
                <div className="text-center mt-3">
