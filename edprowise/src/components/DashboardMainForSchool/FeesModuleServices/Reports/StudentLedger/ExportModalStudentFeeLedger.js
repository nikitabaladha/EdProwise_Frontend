...
export const exportToExcel = async (
    ...
    const worksheet = workbook.addWorksheet('Student Fee Ledger');
    ...
    const fileName = `Student_Fee_Ledger_${student.admissionNo}_${student.studentName.replace(/\s+/g, '_')}.xlsx`;
...
export const exportToPDF = async (
    ...
    `;
      ...
      ${generateHeader(school, logoImg ? logoSrc : '')}
  ...
  <strong>Section:</strong> ${student.section}
    ...
    `;
        ...
        for (let i = 0; i < transactions.length; i += rowsPerPage) {
            pageData.push(transactions.slice(i, i + rowsPerPage));
        }
        ...
        for (let page = 0; page < pageData.length; page++) {
            ...
            const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Student Fee Ledger - ${student.studentName}</div>
        <table>
          <thead>
            <tr>
          ...
          <tbody>
            ${currentPageData.length > 0
                  ...
                  `)
                        .join('')
                    : `<tr><td colspan="${tableFields.length}">No transactions available for the selected student.</td></tr>`
                }
          </tbody>
        </table>
      `;
        ...
        const fileName = `Student_Fee_Ledger_${student.admissionNo}_${student.studentName.replace(/\s+/g, '_')}.pdf`;
        pdf.save(fileName);
    } catch (error) {
