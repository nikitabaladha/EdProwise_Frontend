...
export const exportToExcel = async (
  ...
  try {
    console.log('Starting exportToExcel with filteredData:', filteredData);
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Student Count Report');
    ...
    saveAs(
      ...
      `Student_Count_${formatAcademicYear(selectedAcademicYear)}.xlsx`
    );
  } catch (error) {
...
export const exportToPDF = async (
  ...
  try {
    ...
    for (let page = 0; page < pageData.length; page++) {
      ...
      const tableContent = `
        ${tableStyle}
        <div class="pdf-title">Student Count Report - ${formatAcademicYear(selectedAcademicYear)}</div>
        <table>
          <thead>
            <tr>
    ...
    pdf.save(`Student_Count_${formatAcademicYear(selectedAcademicYear)}.pdf`);
  } catch (error) {
