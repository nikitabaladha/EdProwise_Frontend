...
export const exportToExcel = async (
  ...
  try {
    ...
    Object.keys(classGroups).forEach(className => {
      ...
      const classTotal = classData.reduce(
        (acc, record) => ({
          openingHC: acc.openingHC + (record.openingHC || 0),
          newAdmission: acc.newAdmission + (record.newAdmission || 0),
          leftTC: acc.leftTC + (record.leftTC || 0),
          leftDropout: acc.leftDropout + (record.leftDropout || 0),
          closingHC: acc.closingHC + (record.closingHC || 0),
        }),
        { openingHC: 0, newAdmission: 0, leftTC: 0, leftDropout: 0, closingHC: 0 }
      );
      worksheet.addRow(['Total', '', classTotal.openingHC.toFixed(0), classTotal.newAdmission.toFixed(0), classTotal.leftTC.toFixed(0), classTotal.leftDropout.toFixed(0), classTotal.closingHC.toFixed(0)]);
    });

    worksheet.addRow(['Grand Total', '', totals.openingHC.toFixed(0), totals.newAdmission.toFixed(0), totals.leftTC.toFixed(0), totals.leftDropout.toFixed(0), totals.closingHC.toFixed(0)]);
                  ...
                  .map((row) => {
                    if (row.isTotalRow) {
                      return `
                        <tr class="total-row">
                          <td colspan="2"><strong>Total</strong></td>
                          <td><strong>${(row.totals.openingHC || 0).toFixed(0)}</strong></td>
                          <td><strong>${(row.totals.newAdmission || 0).toFixed(0)}</strong></td>
                          <td><strong>${(row.totals.leftTC || 0).toFixed(0)}</strong></td>
                          <td><strong>${(row.totals.leftDropout || 0).toFixed(0)}</strong></td>
                          <td><strong>${(row.totals.closingHC || 0).toFixed(0)}</strong></td>
                        </tr>
                      `;
                    } else if (row.isGrandTotalRow) {
                      return `
