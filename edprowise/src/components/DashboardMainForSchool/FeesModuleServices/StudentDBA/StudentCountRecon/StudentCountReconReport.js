...
const StudentCountRecon = () => {
  const headerMapping = {
    className: 'Class',
    sectionName: 'Section',
    openingHC: 'Opening HC',
    newAdmission: 'New Admission',
    leftTC: 'Left (TC)',
    leftDropout: 'Left (Dropout)',
    closingHC: 'Closing HC',
  };
    ...
    try {
      ...
      const previousYear = academicYears
        ...
        .find((year) => {
          ...
          return startYear === selectedStartYear - 1;
        });

      const groupedData = data.reduce((acc, record) => {
        const academicHistory = record.academicHistory || [];
        const currentYearRecord = academicHistory.find((h) => h.academicYear === selectedAcademicYear);
        const previousYearRecord = previousYear
          ? academicHistory.find((h) => h.academicYear === previousYear)
          : null;

        if (!currentYearRecord) return acc;

        const className = currentYearRecord.className || '-';
        const sectionName = currentYearRecord.sectionName || '-';
        ...
        if (!previousYearRecord && academicHistory.some((h) => h.academicYear === selectedAcademicYear)) {
          acc[key].newAdmission += 1;
        }

        if (record.TCStatusYear === selectedAcademicYear) {
          acc[key].leftTC += 1;
        }

        if (record.dropoutStatusYear === selectedAcademicYear) {
          acc[key].leftDropout += 1;
        }
      ...
      Object.values(groupedData).forEach((record) => {
        record.closingHC = record.openingHC + record.newAdmission - record.leftTC - record.leftDropout;
      });
  ...
  const classTotals = filteredData.reduce((acc, record) => {
    ...
    acc[className].openingHC += record.openingHC;
    acc[className].newAdmission += record.newAdmission;
    acc[className].leftTC += record.leftTC;
    acc[className].leftDropout += record.leftDropout;
    acc[className].closingHC += record.closingHC;
    return acc;
  }, {});

  const grandTotals = filteredData.reduce(
    (acc, record) => ({
      openingHC: acc.openingHC + record.openingHC,
      newAdmission: acc.newAdmission + record.newAdmission,
      leftTC: acc.leftTC + record.leftTC,
      leftDropout: acc.leftDropout + record.leftDropout,
      closingHC: acc.closingHC + record.closingHC,
    }),
    { openingHC: 0, newAdmission: 0, leftTC: 0, leftDropout: 0, closingHC: 0 }
  );
            ...
            <div className="card-body">
              ...
              <div className="container">
                <div className="card-header d-flex justify-content-between align-items-center gap-1">
                  <h2 className="payroll-title text-center mb-0 flex-grow-1">Student Count Recon Report</h2>
                </div>
              </div>
                    ...
                    <table className="table text-dark border border-secondary mb-1">
                                ...
                                {index === array.length - 1 || record.className !== array[index + 1]?.record.className ? (
                                  <tr className="payroll-table-footer">
                                    ...
                                    <td className="text-center border border-secondary p-2">
                                      <strong>{classTotals[record.className]?.openingHC.toFixed(0) || '0'}</strong>
                                    </td>
                                    <td className="text-center border border-secondary p-2">
                                      <strong>{classTotals[record.className]?.newAdmission.toFixed(0) || '0'}</strong>
                                    </td>
                                    <td className="text-center border border-secondary p-2">
                                      <strong>{classTotals[record.className]?.leftTC.toFixed(0) || '0'}</strong>
                                    </td>
                                    <td className="text-center border border-secondary p-2">
                                      <strong>{classTotals[record.className]?.leftDropout.toFixed(0) || '0'}</strong>
                                    </td>
                                    <td className="text-center border border-secondary p-2">
                                      <strong>{classTotals[record.className]?.closingHC.toFixed(0) || '0'}</strong>
                                    </td>
                                  </tr>
                                ) : null}
                              </React.Fragment>
                      ...
                      <tfoot>
                        <tr className="payroll-table-footer">
                          ...
                          <td className="text-center border border-secondary p-2">
                            <strong>{grandTotals.newAdmission.toFixed(0)}</strong>
                          </td>
                          <td className="text-center border border-secondary p-2">
                            <strong>{grandTotals.leftTC.toFixed(0)}</strong>
                          </td>
                          <td className="text-center border border-secondary p-2">
                            <strong>{grandTotals.leftDropout.toFixed(0)}</strong>
                          </td>
                          <td className="text-center border border-secondary p-2">
                            <strong>{grandTotals.closingHC.toFixed(0)}</strong>
                          </td>
                        </tr>
                      </tfoot>
                    </table>
...
export default StudentCountRecon;