...
const DefineAcademicYear = () => {
  ...
  const fetchAcademicYears = async (schoolId) => {
    try {
      ...
      console.log("academic year", response.data.data);
    } catch (err) {
  ...
  const fetchSettings = async (schoolIdVal, academicYearVal) => {
    try {
      const response = await getAPI(
        `/get-school-payroll-academic-year/${schoolIdVal}/${academicYearVal}`,
        {},
        true
      );
      if (!response.hasError) {
        ...
        console.log("Academic Year Start date", setFormData.startDate);
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Error updating academic year."
      );
    } finally {
      setSending(false);
    }
