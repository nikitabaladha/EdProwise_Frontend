...
const CTCUpdatedEmployeeTable = () => {
  ...
  useEffect(() => {
    ...
    fetchEmployees(id, academicYear);
  }, []);

  useEffect(() => {
      if (schoolId && academicYear) {
        fetchAcademicYears(schoolId);
    fetchEmployees(schoolId, academicYear);
      }
  ...
  const fetchAcademicYears = async (schoolId) => {
       ...
       const fetchEmployees = async (id, academicYear) => {
      try {
        const response = await getAPI(`/getAll-employee-ctc/${id}/${academicYear}`);
        console.log("getAll-employee-ctc",response);
          ...
          <div className="col-xl-12">
            <div className="card m-2">
              <div className="card-body custom-heading-padding">
                ...
                {employeeList.length > employeesPerPage && (
                  <div className="card-footer border-top">
                    <nav aria-label="Page navigation example">
                      <ul className="pagination justify-content-end mb-0">
