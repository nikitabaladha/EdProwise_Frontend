...
const CompleteSellerProfile = () => {
  ...
  const handleSendOTP = async () => {
    if (!formData.emailId.match(/^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/)) {
                      ...
                      <div className="mb-3">
                        ...
                        <span style={{ display: "flex", alignItems: "center", justifyContent: "end" }}>
                          ...
                          <div className="form-label" style={{ position: "absolute", margin: "0 0.9rem 0 0" }}>
                            <div
                              className={`d-inline-block px-2 py-1 rounded ${emailVerificationState === 'verified' ? 'text-success' : 'text-black'} ${!formData.emailId ? 'text-muted' : ''}`}
                              style={{
                                cursor: emailVerificationState === 'verified' || !formData.emailId ? 'default' : 'pointer',
                                opacity: emailVerificationState === 'verified' || !formData.emailId ? 0.7 : 1
                              }}
                              onClick={async () => {
                                if (emailVerificationState !== 'verified' && formData.emailId) {
                                  await handleSendOTP();
                                }
                              }}
                            >
