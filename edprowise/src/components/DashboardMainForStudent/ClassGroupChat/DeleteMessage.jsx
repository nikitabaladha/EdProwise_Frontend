...
const DeleteMessage = ({
  ...
  activeStep,
  handleClick,
  userId = "", // Current user ID (userObjId)
...
const getCurrentUserRole = () => {
  if (!currentConversation || !userId) return null;

  const currentMember = currentConversation.members.find(
    (member) => member.userId.toString() === userId
  );

  return currentMember ? currentMember.userType : null;
};

const isAdmin = getCurrentUserRole() === "Admin";

const isGroup = currentConversation?.isGroup || messages?.receiver?.isGroup;
  ...
  const openLeaveDialog = (conversationId) => {
    console.log("conversationId to leave:", conversationId);
    setSelectedTrainee(conversationId);
...
const handleLeaveConfirmed = async (_id) => {
  try {
    ...
    if (response.data.success) {
      ...
      toast.error(response.data.message || "Failed to leave group");
    }
  } catch (error) {
    ...
    >
        ...
        <>
          ...
          <div className="messenger-infoView-btns ">
            ...
            >
              <div className="d-flex align-items-center mr-4">
                <FaRegTrashAlt className="" />
                <span className="ms-1">Leave Group</span>
              </div>
            </Link> */}
            ...
            {isGroup && isAdmin && (
              ...
              >
                <div className="d-flex align-items-center mr-4">
                  <FaRegTrashAlt className="" />
                  <span className="ms-1">Delete Group</span>
                </div>
              </Link>
            )}
              ...
              {hasPhotos ? (
                ...
                >
                  {currentMessages.messages
                    .filter(
                      (msg) =>
                        msg.messageFile && !msg.messageFile.endsWith(".pdf")
                    )
                    .map((msg, index) => (
                      <div
                        className="message-image"
      ...
      {isDeleteDialogOpen && (
        <ConfirmationDialog
          onClose={handleDeleteCancel}
          // deleteType="conversation"
          deleteType={isGroup ? "group" : "conversation"}
          id={selectedTrainee}
          onDeleted={handleDeleteConfirmed}
        />
      )}
      ...
      {isLeaveDialogOpen && (
        <ConfirmationDialog
          onClose={handleDeleteCancel}
          deleteType="leave-group"
          id={selectedTrainee}
          onDeleted={handleLeaveConfirmed}
