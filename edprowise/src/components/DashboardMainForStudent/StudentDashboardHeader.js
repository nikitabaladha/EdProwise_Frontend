...
const StudentDashboardHeader = () => {
  ...
  const logout = useLogout();
    ...
    }, [schoolId, academicYear, admissionNumber]);
...
const [adminProfile, setAdminProfile] = useState(null);
  ...
  const fetchStudentProfileData = async () => {
    try {
      const response = await getAPI(
        `/get-student-details?admissionNumber=${admissionNumber}&schoolId=${schoolId}&academicYear=${academicYear}`
      );
      console.log("StudentRegisterListTable", response);
      
      if (!response.data.hasError) {
        setStudentProfile(response.data.data);
         toast.success("student get ")
        console.log("student data from heder", response.data.data);
      } else {
        toast.error("student not get")
        console.error("Invalid response format or error in response");
      }
    } catch (err) {
      toast.error("student get error")
  ...
  const navigateToViewAdminProfile = (event, _id) => {
    ...
    navigate("/student-dashboard/view-student-profile", {
      state: { _id },
    });
  };
  ...
  const navigateToChangeAdminPassword = (event, adminProfile) => {
    event.preventDefault();
    navigate("/student-dashboard/change-student-password", {
      state: { adminProfile },
    });
  };
              ...
              <div className="dropdown topbar-item">
                ...
                >
                  <span className="d-flex align-items-center">
                    <img
                      src={`${process.env.REACT_APP_API_URL_FOR_IMAGE}${studentProfile?.studentPhoto}`}
