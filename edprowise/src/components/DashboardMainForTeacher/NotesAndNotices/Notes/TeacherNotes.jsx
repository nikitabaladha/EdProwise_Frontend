...
const TeacherNotes = () => {
  ...
  const navigateToAdd = (event) => {
    ...
    navigate(`/teacher-dashboard/notes-notice/notes/add-note`);
  };

  const navigateToView = (event) => {
    event.preventDefault();
    navigate(`/teacher-dashboard/notes-notice/notes/view-note`);
  };

  const navigateToUpdate = (event) => {
    event.preventDefault();
    navigate(`/teacher-dashboard/notes-notice/notes/update-note`);
  };

  const cardData = [
    {
      icon: <IoNewspaperOutline className="fs-36 text-warning" />,
      title: "Exam start on may-2025",
      info: [
...

    {
      ...
      title: "Book taken from library return on 20-08",
...

    {
      ...
      title: "School Student-body metting Arrangement on 10-08",
      info: [
        "Chairs adjusment",
        "Speech Making",
