...
const TeacherDashboardHeader = () => {
  ...
  const fetchSchoolData = async () => {
    ...
    if (!schoolId || !empId || !academicYear) {
      console.error(
        `School id ${schoolId}, empId ${empId} , academic Year ${academicYear} not found in localStorage`
      );
      return;
    }

    try {
      const employeeRes = await getAPI(
        `/get-employee-details/${schoolId}/${empId}/${academicYear}`
      );
  ...
  const navigateToViewSchoolProfile = (event) => {
    event.preventDefault();
    navigate("/teacher-dashboard/view-teacher-profile", {
      // state: { _id, schoolId },
    });
  };

  const navigateToChangeSchoolPassword = (event) => {
    event.preventDefault();
  ...
  const navigateToView = (event) => {
    event.preventDefault();
    navigate(`/principal-dashboard/view-all-notification`);
  };
